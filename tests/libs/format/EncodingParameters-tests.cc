...
DecoderConfigurationTypeCABAC* getTestingDecoderConfigurationTypeCABAC() {
    ...
    transform_subseq_parameters[4] = constructMergeCodingSubseqTransformSubseqParameters(2, merge_coding_shift_size);
    ...
    cabac_binarizations[1][0] = constructCabacBinarizationBinaryCoding_Bypass();
    {
        ...
        contextInitVals[0] = 0;
        contextInitVals[1] = 1;
        cabac_binarizations[1][1] = constructCabacBinarizationBinaryCoding_NotBypass(true, 2, contextInitVals, false);
    }
    cabac_binarizations[2] = (Cabac_binarizationsType**)malloc(3 * sizeof(Cabac_binarizationsType));
    {
        ...
        contextInitVals[0] = 0;
        contextInitVals[1] = 1;
        cabac_binarizations[2][0] =
            constructCabacBinarizationDoubleTruncatedUnary_NotBypass(2, 3, true, 2, contextInitVals, false);
    }
    cabac_binarizations[2][1] = constructCabacBinarizationExponentialGolomb_Bypass();
    {
        ...
        contextInitVals[0] = 0;
        contextInitVals[1] = 1;
        cabac_binarizations[2][2] =
            constructCabacBinarizationExponentialGolomb_NotBypass(true, 2, contextInitVals, false);
    }
    ...
    cabac_binarizations[3][0] = constructCabacBinarizationSignedExponentialGolomb_Bypass();
    {
        ...
        contextInitVals[0] = 0;
        contextInitVals[1] = 1;
        cabac_binarizations[3][1] =
            constructCabacBinarizationSignedExponentialGolomb_NotBypass(true, 2, contextInitVals, false);
    }
    cabac_binarizations[4] = (Cabac_binarizationsType**)malloc(2 * sizeof(Cabac_binarizationsType));
    {
        ...
        contextInitVals[0] = 0;
        contextInitVals[1] = 1;
        cabac_binarizations[4][0] =
            constructCabacBinarizationTruncatedExponentialGolomb_NotBypass(2, true, 2, contextInitVals, false);
    }
    cabac_binarizations[4][1] = constructCabacBinarizationSignedTruncatedExponentialGolomb_Bypass(2);

    return constructDecoderConfigurationCABAC(static_cast<uint8_t>(num_subsequences - 1), descriptorSubsequenceID,
                                              transform_subseq_parameters, transform_id_subsyms, support_values,
                                              cabac_binarizations);
}
...
void testDecoderConfigurationCabac(DecoderConfigurationTypeCABAC* decoderConfigurationTypeCABAC) {
    ...
    EXPECT_EQ(SUCCESS, getTransformSubSeqId(transform_subseq_parameters_buffer, &transform_ID_subseq_buffer));
    EXPECT_EQ(SubSeq_NO_TRANSFORM, transform_ID_subseq_buffer);
    EXPECT_EQ(FIELDs_EXISTANCE_CONDITIONS_NOT_MET,
    ...
    EXPECT_EQ(SUCCESS, getMatchCodingBufferSize(transform_subseq_parameters_buffer, &match_coding_buffer_size_buffer));
    ...
    EXPECT_EQ(FIELDs_EXISTANCE_CONDITIONS_NOT_MET,
              getMatchCodingBufferSize(transform_subseq_parameters_buffer, &match_coding_buffer_size_buffer));
    EXPECT_EQ(SUCCESS, getRLECodingGuard(transform_subseq_parameters_buffer, &rle_coding_guard_buffer));
    ...
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 0, 0, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 1, 0, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 1, 1, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 2, 0, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 2, 1, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 2, 2, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 3, 0, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 3, 1, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 4, 0, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    EXPECT_EQ(SUCCESS, getSupportValueCodingOrder(decoderConfigurationTypeCABAC, 4, 1, &coding_order_buffer));
    EXPECT_EQ(1, coding_order_buffer);
    ...
    EXPECT_EQ(FIELDs_EXISTANCE_CONDITIONS_NOT_MET,
              getSplitUnitSize(cabac_binarizations_buffer, &split_unit_size_buffer));
...
Decoder_configuration_tokentype* getTestingDecoderConfigurationTokenType() {
    ...
    Decoder_configuration_tokentype_cabac* decoder_configuration_tokentype_cabac0 =
        constructDecoder_configuration_tokentype_cabac(constructNoTransformSubseqParameters(), transformSubSymIdEnum0,
                                                       supportValues0, cabacBinarizations0);
    Decoder_configuration_tokentype_cabac* decoder_configuration_tokentype_cabac1 =
        constructDecoder_configuration_tokentype_cabac(constructNoTransformSubseqParameters(), transformSubSymIdEnum1,
                                                       supportValues1, cabacBinarizations1);
    return constructDecoderConfigurationTokentype(2, decoder_configuration_tokentype_cabac0,
...
TEST_F(encodingParametersTest, binarization) {
    ...
    Cabac_binarizationsType* cabacBinarization = constructCabacBinarizationBinaryCoding_NotBypass(
        expectedAdaptive_mode_flag, expectedNumContexts, argumentContextInitialization, expectedShareSubSymCtxFlag);
    ...
    EXPECT_TRUE(getContextInitializationValues(cabacContextParameters, &numContexts, &contextInitialization) ==
                SUCCESS);
    ...
    EXPECT_TRUE(getcMaxDTU(cabacBinarization, &cMaxDTU) == FIELDs_EXISTANCE_CONDITIONS_NOT_MET);
    EXPECT_TRUE(getSplitUnitSize(cabacBinarization, &splitUnitSize) == FIELDs_EXISTANCE_CONDITIONS_NOT_MET);
    EXPECT_TRUE(getCabacContextParameters(cabacBinarization, &cabacContextParameters) ==
    ...
    cabacBinarization = constructCabacBinarizationTruncatedUnary_NotBypass(
        0, expectedAdaptive_mode_flag, expectedNumContexts, argumentContextInitialization, expectedShareSubSymCtxFlag);
    ...
    cabacBinarization = constructCabacBinarizationTruncatedExponentialGolomb_NotBypass(
        expectedCTruncExpGolParam, expectedAdaptive_mode_flag, expectedNumContexts, argumentContextInitialization,
    ...
    cabacBinarization = constructCabacBinarizationSplitUnitwiseTruncatedUnary_NotBypass(
        expectedSplitUnitSize, expectedAdaptive_mode_flag, expectedNumContexts, argumentContextInitialization,
        expectedShareSubSymCtxFlag);
    EXPECT_TRUE(cabacBinarization != nullptr);
    EXPECT_TRUE(getBinarizationId(cabacBinarization, &binarization_ID) == SUCCESS);
    ...
    cabacBinarization = constructCabacBinarizationDoubleTruncatedUnary_NotBypass(
        expectedCMaxDTU, expectedSplitUnitSize, expectedAdaptive_mode_flag, expectedNumContexts,
        argumentContextInitialization, expectedShareSubSymCtxFlag);
    EXPECT_TRUE(cabacBinarization != nullptr);
    EXPECT_TRUE(getBinarizationId(cabacBinarization, &binarization_ID) == SUCCESS);
    ...
    cabacBinarization =
        constructCabacBinarizationSignedDoubleTruncatedUnary_Bypass(expectedCMaxDTU, expectedSplitUnitSize);
    EXPECT_TRUE(cabacBinarization != nullptr);
    EXPECT_TRUE(getBinarizationId(cabacBinarization, &binarization_ID) == SUCCESS);
    EXPECT_TRUE(binarization_ID == BinarizationID_SignedDoubleTruncatedUnary);
    EXPECT_TRUE(getBypassFlag(cabacBinarization, &bypassFlag) == SUCCESS);
...
TEST_F(encodingParametersTest, decoderConfigurationTypeCABAC) {
    ...
    for (int transformSubSeq = 0; transformSubSeq < 5; transformSubSeq++) {
        for (int symbolSizeInequality = 0; symbolSizeInequality < 2; symbolSizeInequality++) {
            for (int coding_order = 0; coding_order < 2; coding_order++) {
                for (int transform_Id_subsym = 0; transform_Id_subsym < 3; transform_Id_subsym++) {
                    supportValuesCollection[transformSubSeq + 10 * symbolSizeInequality + 5 * coding_order]
                                           [transform_Id_subsym] =
                                               constructSupportValues(4, (uint8_t)(symbolSizeInequality ? 5 : 3),
                                                                      (uint8_t)coding_order, true, true);
                }
            }
        }
    }
    ...
    DecoderConfigurationTypeCABAC* decoderConfigurationTypeCABAC =
        constructDecoderConfigurationCABAC(19, descriptor_subsequence_id, transformSubseqParameters2,
                                           transformSubSymIdEnums, supportValuesCollection, cabacBinarizations);

    uint8_t numDescriptorSubsequenceCfgsMinus1;

    EXPECT_TRUE(getNumDescriptorSubsequenceCfgsMinus1(decoderConfigurationTypeCABAC,
                                                      &numDescriptorSubsequenceCfgsMinus1) == SUCCESS);
    EXPECT_TRUE(numDescriptorSubsequenceCfgsMinus1 == 19);
    for (int transformSubSeq = 0; transformSubSeq < 5; transformSubSeq++) {
        for (int symbolSizeInequality = 0; symbolSizeInequality < 2; symbolSizeInequality++) {
            for (uint8_t coding_order = 0; coding_order < 2; coding_order++) {
                auto descriptorSubSequence = (uint16_t)(transformSubSeq + 10 * symbolSizeInequality + 5 * coding_order);

                uint16_t retrievedId;
                getDescriptorSubsequenceId(decoderConfigurationTypeCABAC, (uint16_t)descriptorSubSequence,
                                           &retrievedId);

                EXPECT_TRUE(descriptorSubSequence == retrievedId);

                Transform_subseq_parametersType* transformSubseqParameter;
                getTransformSubseqParameters(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                             &transformSubseqParameter);

                TransformSubSeqIdEnum resultSubSeqId;
                uint16_t resultBufferSize;
                uint8_t resultCodingGuard;
                EncodingParametersRC rcSubSeqId = getTransformSubSeqId(transformSubseqParameter, &resultSubSeqId);
                EncodingParametersRC rcBufferSize =
                    getMatchCodingBufferSize(transformSubseqParameter, &resultBufferSize);
                EncodingParametersRC rcCodingguard = getRLECodingGuard(transformSubseqParameter, &resultCodingGuard);

                uint8_t transformSubseqCounter;
                EncodingParametersRC rcTransformSubSymCounter = getTransformSubseqCounter(
                    decoderConfigurationTypeCABAC, descriptorSubSequence, &transformSubseqCounter);
                ...
                for (uint16_t transformSubsym_i = 0; transformSubsym_i < transformSubseqCounter; transformSubsym_i++) {
                    ...
                    EXPECT_TRUE(getTransformIdSubSym(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                                     transformSubsym_i, &transformSubSymIdEnum) == SUCCESS);
                    ...
                    EncodingParametersRC retrieved_outputSymbolSize_RC =
                        getSupportValueOutputSymbolSize(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                                        transformSubsym_i, &retrieved_outputSymbolSize);
                    EncodingParametersRC retrieved_codingSymbolSize_RC =
                        getSupportValueCodingSymbolSize(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                                        transformSubsym_i, &retrieved_codingSymbolSize);
                    EncodingParametersRC retrieved_coding_order_RC =
                        getSupportValueCodingOrder(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                                   transformSubsym_i, &retrieved_coding_order);
                    EncodingParametersRC retrieved_shareSubSymLutFlag_RC =
                        getSupportValueShareSubsymLUTFlag(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                                          transformSubsym_i, &retrieved_shareSubSymLutFlag);
                    EncodingParametersRC retrieved_shareSubsymPrvFlag_RC =
                        getSupportValueShareSubsymPRVFlag(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                                          transformSubsym_i, &retrieved_shareSubsymPrvFlag);
                    ...
                    EXPECT_TRUE(getCABACBinarizations(decoderConfigurationTypeCABAC, descriptorSubSequence,
                                                      transformSubsym_i, &retrievedCabacBinarization) == SUCCESS);
...
contructDecoder_configuration_tokentypeCABAC( expected_output_symbol_size_tokentype,
    ...
    Decoder_configuration_tokentype_cabac* decoderConfigurationTokentypeCABAC2 =
contructDecoder_configuration_tokentypeCABAC( expected_output_symbol_size_tokentype,
            expected_coding_symbols_tokentype_numbers,
            expected_coding_symbol_tokentype_size,
            transformSubSymIdEnumToken,
...
TEST_F(encodingParametersTest, constructEncodingParametersSingleAlignmentNoComputedTest) {
    ...
    Encoding_ParametersType* encodingParameters = constructEncodingParametersSingleAlignmentNoComputed(
        ...
        qv_depth, as_depth, numClasses, classIDs_argument, numGroups, (char**)rgroupId_argument, splicedReadsFlag,
        multipleSignatureFlag, multipleSignatureBase, U_signature_size, qv_coding_mode, qvps_flag, parameter_set_qvps,
        qvps_preset_ID);
...
TEST_F(encodingParametersTest, constructEncodingParametersMultipleAlignmentNoComputedTest) {
    ...
    Encoding_ParametersType* encodingParameters = constructEncodingParametersMultipleAlignmentNoComputed(
        datasetType, alphabetId, reads_length, number_of_template_segments_minus1, max_au_data_unit_size, pos40bits,
        qv_depth, as_depth, numClasses, classIDs_argument, numGroups, (char**)rgroupId_argument, splicedReadsFlag,
        multipleSignatureBase, U_signature_size, qv_coding_mode, qvps_flag, parameter_set_qvps, qvps_preset_ID);
...
TEST_F(encodingParametersTest, writeAndReadCabacBinarizationParameters) {
    ...
    Cabac_binarization_parametersType* readValue =
        readCabacBinarizationParameters(&inputBitstream, BinarizationID_TruncatedUnary);
    ...
    testOutput = fopen("test", "wb");
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_ExponentialGolomb);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    EXPECT_EQ(0, ftell(testOutput));
    fclose(testOutput);
    ...
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_SignedExponentialGolomb);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    EXPECT_EQ(0, ftell(testOutput));
    fclose(testOutput);
    ...
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_TruncatedExponentialGolomb);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    EXPECT_EQ(1, ftell(testOutput));
    fclose(testOutput);
    ...
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_SignedTruncatedExponentialGolomb);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    EXPECT_EQ(1, ftell(testOutput));
    fclose(testOutput);
    ...
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_SplitUnitWiseTruncatedUnary);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    EXPECT_EQ(1, ftell(testOutput));
    fclose(testOutput);
    ...
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_SignedSplitUnitWiseTruncatedUnary);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    EXPECT_EQ(1, ftell(testOutput));
    fclose(testOutput);
    ...
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_DoubleTruncatedUnary);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    ...
    initializeOutputBitstream(&outputBitstream, testOutput);
    writeCABACBinarizationParameters(&cabac_binarization_parameters, &outputBitstream,
                                     BinarizationID_SignedDoubleTruncatedUnary);
    writeBuffer(&outputBitstream);
    fflush(testOutput);
    EXPECT_EQ(2, ftell(testOutput));
    fclose(testOutput);
...
TEST_F(encodingParametersTest, writeAndReadCabacContextParameters) {
    ...
    EXPECT_EQ(cabac_context_parameters.context_initialization_value[0],
              read_cabac_context_parametersType->context_initialization_value[0]);
    EXPECT_EQ(cabac_context_parameters.context_initialization_value[1],
    ...
    EXPECT_EQ(cabac_context_parameters.context_initialization_value[1],
              read_cabac_context_parametersType->context_initialization_value[1]);
    EXPECT_EQ(cabac_context_parameters.share_sub_sym_ctx_flag,
              read_cabac_context_parametersType->share_sub_sym_ctx_flag);
}

TEST_F(encodingParametersTest, writeAndReadCabacBinarization) {
    Cabac_binarizationsType* cabacBinarizations = constructCabacBinarizationSignedTruncatedExponentialGolomb_Bypass(5);
    ...
    EXPECT_EQ(cabacBinarizations->cabac_binarization_parameters->cmax_teg,
              read_cabacBinarizations->cabac_binarization_parameters->cmax_teg);

    uint8_t initValues[2] = {3, 6};
    cabacBinarizations =
        constructCabacBinarizationSignedTruncatedExponentialGolomb_NotBypass(5, true, 2, initValues, true);
    ...
    EXPECT_EQ(cabacBinarizations->cabac_binarization_parameters->cmax_teg,
              read_cabacBinarizations->cabac_binarization_parameters->cmax_teg);
    EXPECT_EQ(cabacBinarizations->cabac_context_parameters->num_contexts,
              read_cabacBinarizations->cabac_context_parameters->num_contexts);
    EXPECT_EQ(cabacBinarizations->cabac_context_parameters->context_initialization_value[0],
              read_cabacBinarizations->cabac_context_parameters->context_initialization_value[0]);
    EXPECT_EQ(cabacBinarizations->cabac_context_parameters->context_initialization_value[1],
              read_cabacBinarizations->cabac_context_parameters->context_initialization_value[1]);
    EXPECT_EQ(cabacBinarizations->cabac_context_parameters->adaptive_mode_flag,
...
TEST_F(encodingParametersTest, writeAndReadDecoderConfiguration) {
    Transform_subseq_parametersType transform_subseq_parameters;
    transform_subseq_parameters.transform_ID_subseq = 0;
    Transform_subseq_parametersType* arrayTransformSubseq[] = {&transform_subseq_parameters};
    ...
    InputBitstream inputBitstream;
    initializeInputBitstream(&inputBitstream, inputFile);
    DecoderConfigurationTypeCABAC* read_DecoderConfigurationTypeCABAC =
        readDecoderConfigurationTypeCABAC(&inputBitstream);
    ASSERT_NE(nullptr, read_DecoderConfigurationTypeCABAC);
    EXPECT_EQ(decoderConfigurationTypeCABAC.num_descriptor_subsequence_cfgs_minus1,
              read_DecoderConfigurationTypeCABAC->num_descriptor_subsequence_cfgs_minus1);
    EXPECT_EQ(decoderConfigurationTypeCABAC.descriptor_subsequence_ID[0],
              read_DecoderConfigurationTypeCABAC->descriptor_subsequence_ID[0]);
    EXPECT_EQ((**decoderConfigurationTypeCABAC.support_values)->coding_symbol_size,
              (**(*read_DecoderConfigurationTypeCABAC->support_values)).coding_symbol_size);
    EXPECT_EQ((**decoderConfigurationTypeCABAC.support_values)->output_symbol_size,
              (**(*read_DecoderConfigurationTypeCABAC->support_values)).output_symbol_size);
    EXPECT_EQ(decoderConfigurationTypeCABAC.transform_id_subsym[0][0],
              read_DecoderConfigurationTypeCABAC->transform_id_subsym[0][0]);
    EXPECT_EQ((**decoderConfigurationTypeCABAC.cabac_binarizations)->binarization_ID,
...
TEST_F(encodingParametersTest, writeAndReadDecoderConfigurationTokenTypeCabac) {
    ...
    decoder_configuration_tokentype_cabac.transform_subseq_parametersType = constructNoTransformSubseqParameters();
    TransformSubSymIdEnum transformSubSymId[] = {SubSym_NO_TRANSFORM};
    ...
    support_values[0] = constructSupportValues(4, 3, 1, false, false);
    ...
    EXPECT_EQ(decoder_configuration_tokentype_cabac.transform_subseq_parametersType->transform_ID_subseq,
              read_decoder_configuration_tokentype_cabac->transform_subseq_parametersType->transform_ID_subseq);

    EXPECT_EQ(decoder_configuration_tokentype_cabac.support_values[0]->coding_symbol_size,
              read_decoder_configuration_tokentype_cabac->support_values[0]->coding_symbol_size);
    EXPECT_EQ(decoder_configuration_tokentype_cabac.support_values[0]->output_symbol_size,
              read_decoder_configuration_tokentype_cabac->support_values[0]->output_symbol_size);
    EXPECT_EQ(decoder_configuration_tokentype_cabac.support_values[0]->coding_order,
              read_decoder_configuration_tokentype_cabac->support_values[0]->coding_order);

    EXPECT_EQ(decoder_configuration_tokentype_cabac.transformSubSym[0],
              read_decoder_configuration_tokentype_cabac->transformSubSym[0]);

    EXPECT_EQ(decoder_configuration_tokentype_cabac.cabac_binarizations[0]->binarization_ID,
...
TEST_F(encodingParametersTest, writeEncodingParametersSingleAlignmentNoComputedTest) {
    ...
    for (uint8_t descriptor_i = 0; descriptor_i < 18; descriptor_i++) {
        if (descriptor_i != 11 && descriptor_i != 15) {
            ...
            if (descriptor_i % 2 == 0) {
                ...
                setNonClassSpecificDescriptorConfigurationAndEncodingMode(encodingParameters, descriptor_i,
                                                                          testingDecoderConf, 0);
            } else {
                // use this method if the descriptor configuration is different for all classes
                for (uint8_t class_i = 0; class_i < numClasses; class_i++) {
                    DecoderConfigurationTypeCABAC* testingDecoderConf = getTestingDecoderConfigurationTypeCABAC();
                    setClassSpecificDescriptorConfigurationAndEncodingMode(encodingParameters, class_i, descriptor_i,
                                                                           testingDecoderConf, 0);
                }
            }
        } else {
    ...
    EXPECT_EQ(OUT_OF_BOUNDERIES,
              getCABACDecoderConfiguration(encodingParametersType, 6, 0, &decoder_configuration_cabac_buffer));
    EXPECT_EQ(OUT_OF_BOUNDERIES,
              getCABACDecoderConfiguration(encodingParametersType, 0, 19, &decoder_configuration_cabac_buffer));
    EXPECT_EQ(OUT_OF_BOUNDERIES,
              getCABACDecoderConfiguration(encodingParametersType, 0, 11, &decoder_configuration_cabac_buffer));

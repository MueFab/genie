...
coding::MpeggRawAu generate_subseqs(const se_data &data, uint64_t block_num) {
    coding::MpeggRawAu rdata(format::mpegg_p2::ParameterSet(), 0);
    ...
    if (seq_start != seq_end) {
        // not all unaligned
        rdata.push(coding::GenSub::RLEN, seq_end - seq_start - 1);  // rlen
        rdata.push(coding::GenSub::RTYPE, 5);                       // rtype
        for (uint64_t i = seq_start; i < seq_end; i++)
            rdata.push(coding::GenSub::UREADS, char_to_int[(uint8_t)data.seq[i]]);  // ureads
    }
    uint64_t prevpos = 0, diffpos;
    // Write streams
    for (uint64_t i = start_read_num; i < end_read_num; i++) {
        if (data.flag_arr[i] == true) {
            rdata.push(coding::GenSub::RLEN, data.read_length_arr[i] - 1);          // rlen
            rdata.push(coding::GenSub::RCOMP, rc_to_int[(uint8_t)data.RC_arr[i]]);  // rcomp
            if (i == start_read_num) {
                // Note: In order non-preserving mode, if the first read of
                // the block is a singleton, then the rest are too.
                rdata.get(coding::GenDesc::POS, 0).push(0);  // pos
                prevpos = data.pos_arr[i];
            } else {
                diffpos = data.pos_arr[i] - prevpos;
                rdata.get(coding::GenDesc::POS, 0).push(diffpos);  // pos
                prevpos = data.pos_arr[i];
            }
            if (data.noise_len_arr[i] == 0)
                rdata.get(coding::GenDesc::RTYPE, 0).push(1);  // rtype = P
            else {
                rdata.get(coding::GenDesc::RTYPE, 0).push(3);  // rtype = M
                for (uint16_t j = 0; j < data.noise_len_arr[i]; j++) {
                    rdata.get(coding::GenDesc::MMPOS, 0).push(0);  // mmpos
                    if (j == 0)
                        rdata.get(coding::GenDesc::MMPOS, 1).push(data.noisepos_arr[data.pos_in_noise_arr[i] + j]);
                    else
                        rdata.get(coding::GenDesc::MMPOS, 1)
                            .push(data.noisepos_arr[data.pos_in_noise_arr[i] + j] - 1);  // decoder adds +1
                    rdata.get(coding::GenDesc::MMTYPE, 0).push(0);                       // mmtype = Substitution
                    rdata.get(coding::GenDesc::MMTYPE, 1)
                        .push(char_to_int[(uint8_t)data.noise_arr[data.pos_in_noise_arr[i] + j]]);
                }
                rdata.get(coding::GenDesc::MMPOS, 0).push(1);
            }
        } else {
            rdata.get(coding::GenDesc::RTYPE, 0).push(5);                           // rtype
            rdata.get(coding::GenDesc::RLEN, 0).push(data.read_length_arr[i] - 1);  // rlen
            for (uint64_t j = 0; j < data.read_length_arr[i]; j++) {
                rdata.get(coding::GenDesc::UREADS, 0)
                    .push(char_to_int[(uint8_t)data.unaligned_arr[data.pos_arr[i] + j]]);  // ureads
            }
            rdata.get(coding::GenDesc::POS, 0).push(seq_end - prevpos);             // pos
            rdata.get(coding::GenDesc::RCOMP, 0).push(0);                           // rcomp
            rdata.get(coding::GenDesc::RLEN, 0).push(data.read_length_arr[i] - 1);  // rlen
            rdata.get(coding::GenDesc::RTYPE, 0).push(1);                           // rtype = P
            prevpos = seq_end;
...
coding::MpeggRawAu generate_streams_pe(const se_data &data, const pe_block_data &bdata, uint64_t cur_block_num,
    ...
    if (seq_start != seq_end) {
        // not all unaligned
        rdata.get(coding::GenDesc::RLEN, 0).push(seq_end - seq_start - 1);  // rlen
        rdata.get(coding::GenDesc::RTYPE, 0).push(5);                       // rtype
        for (uint64_t i = seq_start; i < seq_end; i++)
            rdata.get(coding::GenDesc::UREADS, 0).push(char_to_int[(uint8_t)data.seq[i]]);  // ureads
    }
    uint64_t prevpos = 0, diffpos;
    // Write streams
    for (uint32_t i = bdata.block_start[cur_block_num]; i < bdata.block_end[cur_block_num]; i++) {
        ...
        if ((bdata.block_num[current] == bdata.block_num[pair]) &&
            ...
            if (data.flag_arr[current] == false) {
                ...
                rdata.get(coding::GenDesc::RTYPE, 0).push(5);  // rtype
                rdata.get(coding::GenDesc::RLEN, 0)
                ...
                rdata.get(coding::GenDesc::RCOMP, 0).push(0);                                 // rcomp
                rdata.get(coding::GenDesc::RCOMP, 0).push(0);                                 // rcomp
                rdata.get(coding::GenDesc::RLEN, 0).push(data.read_length_arr[current] - 1);  // rlen
                rdata.get(coding::GenDesc::RLEN, 0).push(data.read_length_arr[pair] - 1);     // rlen
                rdata.get(coding::GenDesc::RTYPE, 0).push(1);                                 // rtype = P
                rdata.get(coding::GenDesc::PAIR, 0).push(0);  // pair decoding case same_rec
                bool read_1_first = true;
                uint16_t delta = data.read_length_arr[current];
                rdata.get(coding::GenDesc::PAIR, 1).push(!(read_1_first) + 2 * delta);  // pair
                prevpos = seq_end;
                ...
                rdata.get(coding::GenDesc::RLEN, 0).push(data.read_length_arr[current] - 1);          // rlen
                rdata.get(coding::GenDesc::RLEN, 0).push(data.read_length_arr[pair] - 1);             // rlen
                rdata.get(coding::GenDesc::RCOMP, 0).push(rc_to_int[(uint8_t)data.RC_arr[current]]);  // rcomp
                rdata.get(coding::GenDesc::RCOMP, 0).push(rc_to_int[(uint8_t)data.RC_arr[pair]]);     // rcomp
                if (data.noise_len_arr[current] == 0 && data.noise_len_arr[pair] == 0)
                    ...
                    for (int k = 0; k < 2; k++) {
                        ...
                        rdata.get(coding::GenDesc::MMPOS, 0).push(1);  // mmpos
                    }
                }
            ...
            if (same_block && !read_1_first) {
                rdata.get(coding::GenDesc::PAIR, 0).push(1);  // R1_split
                rdata.get(coding::GenDesc::PAIR, 2).push(bdata.genomic_record_index[pair]);
            } else if (same_block && read_1_first) {
                rdata.get(coding::GenDesc::PAIR, 0).push(2);  // R2_split
                rdata.get(coding::GenDesc::PAIR, 3).push(bdata.genomic_record_index[pair]);
            } else if (!same_block && !read_1_first) {
                rdata.get(coding::GenDesc::PAIR, 0).push(3);  // R1_diff_ref_seq
                rdata.get(coding::GenDesc::PAIR, 4).push(bdata.block_num[pair]);
                rdata.get(coding::GenDesc::PAIR, 6).push(bdata.genomic_record_index[pair]);
            } else {
                rdata.get(coding::GenDesc::PAIR, 0).push(4);  // R2_diff_ref_seq
                rdata.get(coding::GenDesc::PAIR, 5).push(bdata.block_num[pair]);
                rdata.get(coding::GenDesc::PAIR, 7).push(bdata.genomic_record_index[pair]);
            }
        }
    }

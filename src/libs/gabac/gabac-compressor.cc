...
genie::BlockPayloadSet::SubsequencePayload GabacCompressor::compress(const gabac::EncodingConfiguration &conf,
                                                                     genie::MpeggRawAu::SubDescriptor &&in) {
    // Interface to GABAC library
    genie::MpeggRawAu::SubDescriptor data = std::move(in);
    ...
    size_t index = 0;
    genie::BlockPayloadSet::SubsequencePayload out(data.getID());
    for (auto &o : outbuffer) {
        out.add(genie::BlockPayloadSet::TransformedPayload(std::move(o), index++));
    }
    return out;
}
...
void GabacCompressor::flowIn(genie::MpeggRawAu &&t, size_t id) {
    genie::MpeggRawAu raw_aus = std::move(t);
    auto payload = genie::BlockPayloadSet(raw_aus.moveParameters(), raw_aus.getNumRecords());

    for (const auto &desc : raw_aus.getDescriptorStreams()) {
        genie::BlockPayloadSet::DescriptorPayload descriptor_payload(desc.getID());

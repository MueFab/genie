...
#include <mpegg_p2/parameter_set/descriptor_configuration_present/cabac/support_values.h>
...
GabacSeqConfSet::GabacSeqConfSet() {
    ...
    for (const auto &desc : genie::getDescriptors()) {
        conf.emplace_back();
...
const gabac::EncodingConfiguration &GabacSeqConfSet::getConfAsGabac(genie::GenSubIndex sub) const {
...
std::unique_ptr<GabacSeqConfSet::TransformSubseqParameters> GabacSeqConfSet::storeTransParams(
    ...
    using namespace mpegg_p2::desc_conf_pres::cabac;
...
GabacSeqConfSet::TransformIdSubsym GabacSeqConfSet::storeTransform(
    ...
    if (tSeqConf.lutTransformationEnabled && tSeqConf.diffCodingEnabled) {
        UTILS_THROW_RUNTIME_EXCEPTION("LUT and Diff backbone at the same time not supported");
...
std::unique_ptr<GabacSeqConfSet::CabacBinarization> GabacSeqConfSet::storeBinarization(
    ...
    if (tSeqConf.contextSelectionId != gabac::ContextSelectionId::bypass) {
        // TODO insert actual values when adaptive backbone ready
...
void GabacSeqConfSet::storeParameters(mpegg_p2::ParameterSet &parameterSet) const {
    using namespace mpegg_p2::desc_conf_pres;
    using namespace mpegg_p2;
...
const GabacSeqConfSet::DecoderConfigurationCabac &GabacSeqConfSet::loadDescriptorDecoderCfg(
    const GabacSeqConfSet::ParameterSet &parameterSet, genie::GenDesc descriptor_id) {
    using namespace mpegg_p2::desc_conf_pres;
    using namespace mpegg_p2;
...
void GabacSeqConfSet::loadParameters(const mpegg_p2::ParameterSet &parameterSet) {
    using namespace mpegg_p2::desc_conf_pres;
    using namespace mpegg_p2::desc_conf_pres::cabac;

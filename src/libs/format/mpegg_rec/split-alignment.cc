...
namespace format {
    namespace mpegg_rec {
        SplitAlignment::SplitAlignment(
                SplitAlignmentType _split_alignment
        ) : split_alignment(_split_alignment) {

        }

        void SplitAlignment::write(util::BitWriter *writer) const {
            writer->write(uint8_t(split_alignment), 8);
        }

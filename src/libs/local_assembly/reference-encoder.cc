...
std::string LocalAssemblyReferenceEncoder::preprocess(const std::string &read, const std::string &cigar) {
    ...
    for (char cigar_pos : cigar) {
        ...
        if (getAlphabetProperties(genie::AlphabetID::ACGTN).isIncluded(cigar_pos) && count == 0) {
...
void LocalAssemblyReferenceEncoder::addRead(const genie::mpegg_rec::MpeggRecord &s) {
    ...
    const auto &rec = dynamic_cast<const genie::mpegg_rec::SplitAlignmentSameRec &>(*ptr);

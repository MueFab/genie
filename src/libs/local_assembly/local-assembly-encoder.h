...
#include <mutex>
#include "backbone/mpegg-encoder.h"
#include "mpegg_read/read-encoder.h"
 ...
 */
class LocalAssemblyEncoder : public genie::MpeggEncoder {
   private:
    ...
    struct LaeState {
        ...
        mpegg_read::LocalAssemblyReadEncoder readCoder;  //!< @brief Generating the descriptor streams
        bool pairedEnd;                                  //!< @brief Current guess regarding pairing
        size_t readLength;                               //!< @brief Current guess regarding read length
        genie::mpegg_rec::ClassType classType;           //!< @brief Current guess regarding class type
        uint64_t minPos;
        uint64_t maxPos;
    };
    ...
    void printDebug(const LocalAssemblyEncoder::LaeState& state, const std::string& ref1, const std::string& ref2,
                    const genie::mpegg_rec::MpeggRecord& r) const;
     ...
     * @param state Current local assembly state
     */
    void updateAssembly(const genie::mpegg_rec::MpeggRecord& r, LocalAssemblyEncoder::LaeState& state) const;
     ...
     */
    const genie::mpegg_rec::SplitAlignmentSameRec& getPairedAlignment(const LocalAssemblyEncoder::LaeState& state,
                                                                      const genie::mpegg_rec::MpeggRecord& r) const;
     ...
     */
    void updateGuesses(const genie::mpegg_rec::MpeggRecord& r, LocalAssemblyEncoder::LaeState& state) const;
     ...
     * @return MpeggRawAu ready for entropy backbone
     */
    genie::MpeggRawAu pack(size_t id, uint16_t ref, LocalAssemblyEncoder::LaeState& state) const;
   ...
   public:
    ...
    static uint64_t getLengthOfCigar(const std::string& cigar) {
        ...
        for (const auto& c : cigar) {
            ...
            if (getAlphabetProperties(genie::AlphabetID::ACGTN).isIncluded(c)) {
                length++;
                digits.clear();
                continue;
            }

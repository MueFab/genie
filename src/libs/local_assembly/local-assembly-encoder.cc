...
#include <mpegg_p2/parameter_set/qv_coding_config_1/qv_coding_config_1.h>
...
LocalAssemblyEncoder::LaeState::LaeState(size_t cr_buf_max_size)
    : refCoder(cr_buf_max_size),
      ...
      pairedEnd(false),
      readLength(0),
      classType(genie::mpegg_rec::ClassType::NONE),
...
void LocalAssemblyEncoder::printDebug(const LocalAssemblyEncoder::LaeState& state, const std::string& ref1,
                                      const std::string& ref2, const genie::mpegg_rec::MpeggRecord& r) const {
    if (!debug) {
        return;
    }
...
void LocalAssemblyEncoder::updateAssembly(const genie::mpegg_rec::MpeggRecord& r,
...
const genie::mpegg_rec::SplitAlignmentSameRec& LocalAssemblyEncoder::getPairedAlignment(
    const LocalAssemblyEncoder::LaeState& state, const genie::mpegg_rec::MpeggRecord& r) const {
    if (!state.pairedEnd) {
        UTILS_DIE("Record is single ended");
    }
    if (r.getAlignments().front().getSplitAlignments().front()->getType() ==
        genie::mpegg_rec::SplitAlignment::SplitAlignmentType::SAME_REC) {
        return *reinterpret_cast<const genie::mpegg_rec::SplitAlignmentSameRec*>(
...
void LocalAssemblyEncoder::updateGuesses(const genie::mpegg_rec::MpeggRecord& r,
                                         LocalAssemblyEncoder::LaeState& state) const {
...
genie::MpeggRawAu LocalAssemblyEncoder::pack(size_t id, uint16_t ref, LocalAssemblyEncoder::LaeState& state) const {
    mpegg_p2::DataUnit::DatasetType dataType = mpegg_p2::DataUnit::DatasetType::ALIGNED;
    mpegg_p2::ParameterSet ret(id, id, dataType, genie::AlphabetID::ACGTN, state.readLength, state.pairedEnd, false, 0,
                               0, false, false);
    const auto ALPHABET = mpegg_p2::qv_coding1::QvCodingConfig1::QvpsPresetId::ASCII;
    ret.addClass(state.classType, util::make_unique<mpegg_p2::qv_coding1::QvCodingConfig1>(ALPHABET, false));
    auto crps = util::make_unique<mpegg_p2::ParameterSetCrps>(mpegg_p2::ParameterSetCrps::CrAlgId::LOCAL_ASSEMBLY);
    crps->setCrpsInfo(util::make_unique<mpegg_p2::CrpsInfo>(0, cr_buf_max_size));
...
void LocalAssemblyEncoder::flowIn(genie::mpegg_rec::MpeggChunk&& t, size_t id) {

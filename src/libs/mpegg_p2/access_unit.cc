...
namespace genie {
namespace mpegg_p2 {
AccessUnit::AccessUnit(const std::map<size_t, std::unique_ptr<mpegg_p2::ParameterSet>> &parameterSets,
    ...
    au_type = genie::mpegg_rec::ClassType(bitReader.read(4));
    reads_count = bitReader.read(32);
    if (au_type == genie::mpegg_rec::ClassType::CLASS_N || au_type == genie::mpegg_rec::ClassType::CLASS_M) {
    ...
    if (au_type != genie::mpegg_rec::ClassType::CLASS_U) {
        this->au_Type_U_Cfg =
...
AccessUnit::AccessUnit(uint32_t _access_unit_ID, uint8_t _parameter_set_ID, genie::mpegg_rec::ClassType _au_type,

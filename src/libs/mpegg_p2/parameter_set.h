...
#include <memory>
#include <vector>
#include "mpegg_p2/parameter_set/parameter_set_crps.h"
...
class ParameterSet : public DataUnit {
   public:
    ...
    ParameterSet(uint8_t _parameter_set_ID, uint8_t _parent_parameter_set_ID, DatasetType _dataset_type,
                 genie::AlphabetID _alphabet_id, uint32_t _read_length, bool _paired_end, bool _pos_40_bits_flag,
                 uint8_t _qv_depth, uint8_t _as_depth, bool _multiple_alignments_flag, bool _spliced_reads_flag);

    ParameterSet();

    const ParameterSetCrps &getParameterSetCrps() const;

    size_t getNumberTemplateSegments() const;

    void setCrps(std::unique_ptr<ParameterSetCrps> parameter_set_crps);

    void addClass(genie::mpegg_rec::ClassType class_id, std::unique_ptr<QvCodingConfig> conf);

    void setDescriptor(genie::GenDesc index, std::unique_ptr<DescriptorConfigurationContainer> descriptor);

    const DescriptorConfigurationContainer *getDescriptor(genie::GenDesc index) const;
   ...
   private:
    ...
    genie::AlphabetID alphabet_ID : 8;               //!< Line 3
    ...
    std::vector<genie::mpegg_rec::ClassType> class_IDs;                          //!< : 4; For loop Lines 12 + 13

...
uint32_t LocalAssemblyReadDecoder::lengthOfNextRead() { return container->rlen_0.get(0); }

uint32_t LocalAssemblyReadDecoder::offsetOfNextRead() { return container->pos_0.get(0); }

uint32_t LocalAssemblyReadDecoder::lengthOfSecondNextRead() { return container->rlen_0.get(1); }

uint32_t LocalAssemblyReadDecoder::offsetOfSecondNextRead() { return container->pos_0.get(1); }
...
void LocalAssemblyReadDecoder::decodePair(const std::string& ref1, format::sam::SamRecord* s1, const std::string& ref2,
...
LocalAssemblyReadDecoder::LocalAssemblyReadDecoder(std::unique_ptr<StreamContainer> _container)
    : posCounter(0), container(std::move(_container)) {}

...
class LocalAssemblyReadEncoder {
   private:
    ...
    void encodeCigar(const std::string &read, const std::string &cigar, const std::string &ref,
    ...
    struct CodingState {
        CodingState(const std::string &_read, const std::string &_ref, format::mpegg_rec::MpeggRecord::ClassType _type,
                    ClipInformation *_clips)
            : count(0),
              read_pos(0),
              ref_offset(0),
              lastMisMatch(0),
        ...
        const format::mpegg_rec::MpeggRecord::ClassType type;
        ClipInformation *clips;
    };

    void codeInsertion(CodingState *state);

    void codeDeletion(CodingState *state);

    static void codeHardClip(CodingState *state);

    static void codeSoftClip(CodingState *state);
   ...
   public:
    ...
    std::unique_ptr<MpeggRawAu> moveStreams();
};

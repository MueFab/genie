...
LocalAssemblyReadEncoder::LocalAssemblyReadEncoder() : container(mpegg_p2::ParameterSet(), 0), pos(0), readCounter(0) {}

// ---------------------------------------------------------------------------------------------------------------------

void LocalAssemblyReadEncoder::encodeFirstSegment(const genie::mpegg_rec::MpeggRecord &rec) {
    // TODO: Splices
    ...
    container.push(genie::GenSub::RTYPE, uint8_t(rec.getClassID()));

    const auto POSITION = ALIGNMENT.getPosition() - pos;
    pos = ALIGNMENT.getPosition();
    container.push(genie::GenSub::POS_MAPPING_FIRST, POSITION);

    const auto LENGTH = RECORD.getSequence().length() - 1;
    container.push(genie::GenSub::RLEN, LENGTH);

    const auto RCOMP = ALIGNMENT.getAlignment().getRComp();
    container.push(genie::GenSub::RCOMP, RCOMP);

    const auto FLAG_PCR =
        (rec.getFlags() & genie::GenConst::FLAGS_PCR_DUPLICATE_MASK) >> genie::GenConst::FLAGS_PCR_DUPLICATE_POS;
    container.push(genie::GenSub::FLAGS_PCR_DUPLICATE, FLAG_PCR);
    const auto FLAG_QUAL =
        (rec.getFlags() & genie::GenConst::FLAGS_QUALITY_FAIL_MASK) >> genie::GenConst::FLAGS_QUALITY_FAIL_POS;
    container.push(genie::GenSub::FLAGS_QUALITY_FAIL, FLAG_QUAL);
    const auto FLAG_PAIR =
        (rec.getFlags() & genie::GenConst::FLAGS_PROPER_PAIR_MASK) >> genie::GenConst::FLAGS_PROPER_PAIR_POS;
    container.push(genie::GenSub::FLAGS_PROPER_PAIR, FLAG_PAIR);

    const auto MSCORE = ALIGNMENT.getAlignment().getMappingScores().front();  // TODO: Multiple mapping scores
    container.push(genie::GenSub::MSCORE, MSCORE);

    const auto RGROUP = 0;  // TODO
    container.push(genie::GenSub::RGROUP, RGROUP);
}

// ---------------------------------------------------------------------------------------------------------------------

const genie::mpegg_rec::SplitAlignmentSameRec &LocalAssemblyReadEncoder::extractPairedAlignment(
    ...
    const auto SAME_REC = genie::mpegg_rec::SplitAlignment::SplitAlignmentType::SAME_REC;
    if (rec.getAlignments().front().getSplitAlignments().front()->getType() == SAME_REC) {
        ...
        return *reinterpret_cast<const genie::mpegg_rec::SplitAlignmentSameRec *>(ALIGNMENT);
    } else {
...
void LocalAssemblyReadEncoder::encodeAdditionalSegment(size_t length,
                                                       const genie::mpegg_rec::SplitAlignmentSameRec &srec) {
    ...
    container.push(genie::GenSub::PAIR_DECODING_CASE, genie::GenConst::PAIR_SAME_RECORD);
    ...
    container.push(genie::GenSub::PAIR_SAME_REC, SAME_REC_DATA);
}

// ---------------------------------------------------------------------------------------------------------------------

void LocalAssemblyReadEncoder::add(const genie::mpegg_rec::MpeggRecord &rec, const std::string &ref1,
    ...
    if (rec.getRecordSegments().size() > 1) {
        const genie::mpegg_rec::SplitAlignmentSameRec &srec = extractPairedAlignment(rec);
...
void LocalAssemblyReadEncoder::encodeInsertion(CodingState &state) {
    for (size_t i = 0; i < state.count; ++i) {
        container.push(genie::GenSub::MMPOS_TERMINATOR, genie::GenConst::MMPOS_PERSIST);
        ...
        container.push(genie::GenSub::MMPOS_POSITION, POSITION);

        container.push(genie::GenSub::MMTYPE_TYPE, genie::GenConst::MMTYPE_INSERTION);

        const auto SYMBOL = getAlphabetProperties(genie::AlphabetID::ACGTN).inverseLut[state.read[state.read_pos]];
        state.read_pos++;
        container.push(genie::GenSub::MMTYPE_INSERTION, SYMBOL);
    }
}
...
void LocalAssemblyReadEncoder::encodeDeletion(CodingState &state) {
    for (size_t i = 0; i < state.count; ++i) {
        ...
        container.push(genie::GenSub::MMPOS_POSITION, POSITION);
        container.push(genie::GenSub::MMTYPE_TYPE, genie::GenConst::MMTYPE_DELETION);
        state.ref_offset++;
    }
}
...
void LocalAssemblyReadEncoder::encodeSoftClip(CodingState &state) {
    for (size_t i = 0; i < state.count; ++i) {
        ...
        state.clips.softClips[state.isRightClip] +=
            getAlphabetProperties(genie::AlphabetID::ACGTN).inverseLut[state.read[state.read_pos]];
        state.read_pos++;
    }
}
...
void LocalAssemblyReadEncoder::encodeSubstitution(CodingState &state) {
    if (state.ref[state.ref_offset] == 0) {
        const auto SYMBOL = getAlphabetProperties(genie::AlphabetID::ACGTN).inverseLut[state.read[state.read_pos]];
        container.push(genie::GenSub::UREADS, SYMBOL);
        return;
    }
    container.push(genie::GenSub::MMPOS_TERMINATOR, genie::GenConst::MMPOS_PERSIST);
    ...
    container.push(genie::GenSub::MMTYPE_TYPE, genie::GenConst::MMTYPE_SUBSTITUTION);
    if (state.type > genie::mpegg_rec::ClassType::CLASS_N) {
        const auto SYMBOL = getAlphabetProperties(genie::AlphabetID::ACGTN).inverseLut[state.read[state.read_pos]];
        container.push(genie::GenSub::MMTYPE_SUBSTITUTION, SYMBOL);
    }
}
...
void LocalAssemblyReadEncoder::encodeCigarToken(char cigar_char, CodingState &state) {
    if (getAlphabetProperties(genie::AlphabetID::ACGTN).isIncluded(cigar_char)) {  // TODO: other alphabets
...
LocalAssemblyReadEncoder::ClipInformation LocalAssemblyReadEncoder::encodeCigar(const std::string &read,
    ...
    if (type > genie::mpegg_rec::ClassType::CLASS_P) {
        container.push(genie::GenSub::MMPOS_TERMINATOR, genie::GenConst::MMPOS_TERMINATE);
    }
    return state.clips;
}
...
genie::MpeggRawAu &&LocalAssemblyReadEncoder::moveStreams() { return std::move(container); }
...
bool LocalAssemblyReadEncoder::encodeSingleClip(const ClipInformation &inf, bool last) {
    ...
    if (clips_present) {
        container.push(genie::GenSub::CLIPS_RECORD_ID, readCounter);
    } else {
    ...
    for (size_t index = 0; index < inf.softClips.size(); ++index) {
        if (!inf.softClips[index].empty()) {
            ...
            container.push(genie::GenSub::CLIPS_TYPE, TYPE);
            for (const auto &c : inf.softClips[index]) {
                container.push(genie::GenSub::CLIPS_SOFT_STRING, c);
            }
            const auto TERMINATOR =
                getAlphabetProperties(genie::AlphabetID::ACGTN).lut.size();  // TODO: other alphabets
            container.push(genie::GenSub::CLIPS_SOFT_STRING, TERMINATOR);
        } else if (inf.hardClips[index]) {
            const auto TYPE = 0x4u | (uint32_t(last) << 1u) | index;
            container.push(genie::GenSub::CLIPS_TYPE, TYPE);
            container.push(genie::GenSub::CLIPS_HARD_LENGTH, inf.hardClips[index]);
        }
    }
...
void LocalAssemblyReadEncoder::encodeClips(const std::pair<ClipInformation, ClipInformation> &clips) {
    ...
    if (present) {
        container.push(genie::GenSub::CLIPS_TYPE, genie::GenConst::CLIPS_RECORD_END);
    }

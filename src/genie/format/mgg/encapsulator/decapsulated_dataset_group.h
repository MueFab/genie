 ...
 * @file
 * @brief Defines the `DecapsulatedDatasetGroup` class for handling the
 * decapsulation of MPEG-G dataset groups.
 *
 * The `DecapsulatedDatasetGroup` class is part of the MPEG-G
 * encapsulation/decapsulation library. It provides methods to extract and
 * manage dataset groups and related metadata from encapsulated MPEG-G dataset
 * files. This class is primarily used for processing and transforming MPEG-G
 * dataset groups into a format suitable for further processing or
 * re-encapsulation.
 *
 * @copyright
 ...
 *
 * The `DecapsulatedDatasetGroup` class is used to transform MPEG-G dataset
 * groups from their encapsulated format into decapsulated components, including
 * metadata, reference sequences, and individual datasets. This decapsulation
 * allows easy access to the internal components for further processing or
 * re-encapsulation.
 */
class DecapsulatedDatasetGroup {
 private:
  uint64_t
      id{};  //!< @brief Unique identifier for the decapsulated dataset group.
  ...
  // Map storing datasets, indexed by dataset ID, along with the associated
  // metadata.
  std::map<uint64_t,
           std::pair<genie::format::mgb::MgbFile, genie::core::meta::Dataset>>
      data;
   ...
   */
  static std::optional<genie::core::meta::DatasetGroup>
  decapsulate_dataset_group(genie::format::mgg::DatasetGroup* grp);
   ...
   * @return A pair containing the decapsulated `AccessUnit` and optional
   * metadata.
   */
  static std::pair<genie::format::mgb::AccessUnit,
                   std::optional<genie::core::meta::AccessUnit>>
  DecapsulateAu(genie::format::mgg::AccessUnit& au);

  /**
   * @brief Decapsulates a dataset and its associated metadata.
   *
   * @param dt The encapsulated `Dataset` to decapsulate.
   * @param meta_group Optional reference to the associated `DatasetGroup`
   * metadata.
   * @param meta_references Map of reference IDs to associated `Reference`
   * metadata.
   * @param grp Pointer to the parent `DatasetGroup`.
   * @return A pair containing the decapsulated `MgbFile` and associated
   * `Dataset` metadata.
   */
  static std::pair<genie::format::mgb::MgbFile, genie::core::meta::Dataset>
  decapsulate_dataset(
      genie::format::mgg::Dataset& dt,
      const std::optional<genie::core::meta::DatasetGroup>& meta_group,
      std::map<uint8_t, genie::core::meta::Reference>& meta_references,
      genie::format::mgg::DatasetGroup* grp);
 ...
 public:
   ...
   *
   * Constructs a `DecapsulatedDatasetGroup` by transferring ownership of
   * internal data from another instance.
   *
  ...
  /**
   * @brief Constructs a `DecapsulatedDatasetGroup` from an encapsulated dataset
   * group.
   *
   * This constructor decapsulates the provided `DatasetGroup` and extracts its
   * components into separate fields for easier manipulation.
   *
   ...
   *
   * @return Map of dataset IDs to associated `MgbFile` and `Dataset` metadata
   * pairs.
   */
  std::map<uint64_t,
           std::pair<genie::format::mgb::MgbFile, genie::core::meta::Dataset>>&

/**
 * @file
 * @brief Defines the `Fasta` class for managing external FASTA reference locations in MPEG-G files.
 *
 * The `Fasta` class is a specialized type of `External` location for referencing sequences stored in a
 * FASTA file format. It includes functionality for managing and validating checksums associated with each sequence.
 * This class is used in the context of MPEG-G reference metadata, allowing efficient referencing and
 * management of external sequence data.
 *
 * @copyright
 ...
 *
 * The `Fasta` class extends the `External` location type to support external references to FASTA files.
 * It provides functionality for managing sequence-level checksums, decapsulation, and serialization.
 * The class is primarily used for referencing large genomic sequences stored externally in FASTA format.
 */
class Fasta : public External {
 private:
    std::vector<std::string> seq_checksums;  //!< @brief Checksums for individual sequences in the FASTA file.

 public:
    /**
     * @brief Decapsulates the `Fasta` location into a core metadata object.
     *
     * This method extracts the internal state of the `Fasta` object and converts it into a more generic
     ...
     * @param _uri The URI pointing to the external FASTA file.
     ...
     *
     * This constructor initializes the `Fasta` object by reading its state from a `BitReader`. The sequence count
     * is used to determine the number of checksums to read.
     *
     ...
     * @param seq_count The number of sequences expected in the FASTA file.
     */
    Fasta(util::BitReader& reader, uint8_t _reserved, std::string _uri, ChecksumAlgorithm algo, size_t seq_count);
     ...
     *
     * This method is part of the `External` interface and is used to add a checksum to the `Fasta` object.
     *
     * @param checksum The checksum value to add.
     */

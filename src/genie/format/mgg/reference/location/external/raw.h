 ...
 * @file
 * @brief Defines the `Raw` class for managing external raw reference locations
 * in MPEG-G files.
 *
 * The `Raw` class is a specialized type of `External` reference location that
 * represents raw sequences in external files. It handles the serialization and
 * deserialization of raw reference data and manages the sequence-level
 * checksums for validating the integrity of these references. This class is
 * used in the context of MPEG-G reference metadata to refer to raw sequences
 * stored externally.
 *
 * @copyright
 ...
 *
 * The `Raw` class extends the `External` location type to support references to
 * raw sequence data stored outside the MPEG-G file. It manages URI
 * references and sequence-level checksums for raw sequences, ensuring data
 * integrity and compatibility. This class is used to represent raw sequence
 * references that are not in standard formats like FASTA or MPEG-G.
 */
class Raw final : public External {
 ...
 public:
   ...
   *
   * This method extracts the internal state of the `Raw` object and converts it
   * into a more generic core metadata representation, which can be used in
   * various contexts within the MPEG-G standard.
   *
   ...
   * @param uri The URI pointing to the external raw reference.
   * @param algo The checksum algorithm used for validating the sequence data.
   */
  Raw(uint8_t reserved, std::string uri, ChecksumAlgorithm algo);

  /**
   * @brief Constructs a `Raw` reference by reading from a bitstream.
   *
   * This constructor initializes the `Raw` object by reading its state from a
   ...
   */
  Raw(util::BitReader& reader, size_t seq_count);
   ...
   * @param seq_count The number of sequences expected in the raw reference.
   */
  Raw(util::BitReader& reader, uint8_t reserved, std::string uri,

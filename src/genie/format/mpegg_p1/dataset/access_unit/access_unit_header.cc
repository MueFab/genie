...
AccessUnitHeader::AccessUnitHeader(util::BitReader& reader, FileHeader&, size_t start_pos, size_t length,
                                   DatasetHeader& dhd)
    : signature_cfgs() {
    ...
    UTILS_DIE_IF(!reader.isAligned() || reader.getPos() - start_pos != length, "Invalid AccessUnitHeader length!");
}
...
void AccessUnitHeader::write(util::BitWriter& writer, bool zero_length) const {
    ...
    if (!MIT_flag) {
        if (au_type_cfg != nullptr) {
            ...
            for (const auto& signature_cfg : signature_cfgs) {

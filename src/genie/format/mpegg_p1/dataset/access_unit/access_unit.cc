...
AUInformation::AUInformation(util::BitReader& reader, FileHeader& fhd, size_t start_pos, size_t length)
    ...
    for (size_t ibyte = 0; ibyte < length - (4 + 8); ibyte++) {
...
#endif
    UTILS_DIE_IF(!reader.isAligned() || reader.getPos() - start_pos != length, "Invalid AUInformation length!");
}

// ---------------------------------------------------------------------------------------------------------------------

uint64_t AUInformation::getLength() const { return AU_information_value.size(); }
...
#endif
    UTILS_DIE_IF(!reader.isAligned() || reader.getPos() - start_pos != length, "Invalid AUProtection length!");
}

// ---------------------------------------------------------------------------------------------------------------------

uint64_t AUProtection::getLength() const { return AU_protection_value.size(); }
...
AccessUnit::AccessUnit(util::BitReader& reader, FileHeader& fhd, size_t start_pos, size_t length, DatasetHeader& dhd)
    ...
    UTILS_DIE_IF(box_key != "auhd", "Access unit header is not found!");
...
#endif
    UTILS_DIE_IF(!reader.isAligned() || (reader.getPos() - start_pos != length), "Invalid AccessUnit length!");
}

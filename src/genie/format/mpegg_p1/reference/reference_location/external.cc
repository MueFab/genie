...
External::External()
    : ReferenceLocation(ReferenceLocation::Flag::EXTERNAL),
      ref_uri(),
      checksum_alg(Checksum::Algo::UNKNOWN),
      external_reference(nullptr) {}

// ---------------------------------------------------------------------------------------------------------------------

External::External(const External& other) : ReferenceLocation(other) { *this = other; }
...
External::External(util::BitReader& reader, FileHeader& fhd, uint16_t seq_count)
    ...
    if (reference_type == ExternalReference::Type::MPEGG_REF) {
        ...
        uint64_t readsize = reader.getPos() - startpos;
...
ExternalReference& External::getExternalRef() const { return *(external_reference.get()); }

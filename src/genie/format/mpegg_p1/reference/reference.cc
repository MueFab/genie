...
Reference::Reference(util::BitReader& reader, FileHeader& fhd, size_t start_pos, size_t length)
    ...
    UTILS_DIE_IF(!reader.isAligned() || reader.getPos() - start_pos != length, "Invalid Reference length!");
}
...
// void Reference::addReferenceLocation(std::unique_ptr<ReferenceLocation> _ref_loc) { reference_location =
// std::move(_ref_loc); }
...
uint64_t Reference::getLength() const {
    ...
    /// sequence_name[] st(v)
    for (size_t seqID = 0; seqID < seq_count; seqID++) {
        /// sequence_name[seqID] st(v)

 ...
 *
 * The `Quantizer` class provides functionality to map quality values into
 * indices and reconstruction values using a forward and inverse lookup table
 * (LUT). It is used to convert the high-resolution quality values into a more
 * compact representation that can be easily stored and transmitted.
 *
 * @details The quantizer class is equipped with methods for bidirectional
 * mapping between quality values and their indices, as well as reconstruction
 * values. The LUTs are constructed based on the mapping specifications provided
 * during initialization. The class can print the LUTs for debugging purposes
 * and supports various mapping and lookup functions.
 */
class Quantizer {
 public:
  /**
   ...
   *
   * Initializes an empty `Quantizer` with no predefined mapping. The LUTs need
   * to be populated manually or through the specialized constructor.
   */
  Quantizer();
   ...
   *
   * This method uses the forward LUT to map a given quality value to its
   * respective index. The index is a more compact representation of the quality
   * value used for encoding.
   *
   ...
   * value. The reconstruction value is used to approximate the original quality
   * value.
   *
   ...
   */
  [[nodiscard]] const std::map<size_t, int>& InverseLut() const;

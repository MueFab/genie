 ...
 * @brief Declaration of the RecordPileup class for handling quality value pileups.
 *
 ...
 * querying records and pre-processing sequences to ensure accurate representation
 * of the data during encoding operations.
 *
 ...
 * positions of all records, allowing for quick lookups and optimized processing.
 * The pileups generated by the class are essential in ensuring accurate and
 * consistent encoding of genomic data.
 */
class RecordPileup {
 private:
    std::vector<EncodingRecord> records;  //!< @brief List of genomic records.
    std::vector<std::vector<std::string>>
        preprocessed_qvalues;  //!< @brief Pre-processed quality values for each record.
    std::vector<std::vector<std::string>> preprocessed_sequences;  //!< @brief Pre-processed sequences for each record.
    uint64_t minPos;  //!< @brief Minimum genomic position of records in the pileup.
    uint64_t maxPos;  //!< @brief Maximum genomic position of records in the pileup.
 ...
 public:
     ...
     * The minimum and maximum positions are set to `std::numeric_limits<uint64_t>::max()`
     * and `std::numeric_limits<uint64_t>::min()` respectively.
     */
    RecordPileup();
     ...
     *
     * This method returns the pileup of sequences and quality values for a specified
     * genomic position. The pileup is represented as a pair of strings, with the first
     * string being the sequence pileup and the second being the corresponding quality values.
     *
     * @param pos The genomic position for which the pileup is requested.
     * @return A pair of strings representing the sequence and quality value pileup.
     */
     ...
     *
     * The given record is added to the internal storage and its quality values and
     * sequences are pre-processed for efficient retrieval during pileup generation.
     *
     ...
     *
     * This static method checks if the records defined by their positions and sequences
     * terminate before a specified genomic position. This is used to filter out records
     * that are not relevant for further pileup processing.
     *
     * @param positions A vector of genomic positions for the records.
     * @param seqs A vector of sequences corresponding to the positions.
     * @param pos The position to compare against.
     * @return `true` if all records end before the given position, `false` otherwise.
     */
     ...
     * genomic position. The records are removed from the internal storage and returned
     * for further processing or cleanup.
     *
     ...
     *
     * This method returns a copy of all records stored in the pileup for inspection or
     * processing. It does not modify the internal storage.
     *
 ...
 private:
     ...
     *
     * This method uses the CIGAR string to generate an expanded version of the sequence,
     * accounting for all insertions, deletions, and skipped bases. It is used to ensure
     * that sequences are correctly aligned during pileup generation.
     *

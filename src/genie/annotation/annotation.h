 ...
 public:
    void startStream(RecType recType, std::string recordInputFileName, std::string attributeJsonFileName, std::string outputFileName) {
        ...
        if (recType == RecType::GENO_FILE) {
            ...
            genie::variant_site::VariantSiteParser parser(inputfile, attributeFields, defaultTileSize);
            auto infoFields = attributeParser.getInfoFields();
            ...
            for (uint64_t i = 0; i < parser.getNrOfTiles(); ++i) {
                ...
                accessUnit.setAccessUnit(desc, attr, parser.getAttributes().getInfo(), annotationParameterSet, annotationAccessUnit.at(i), AG_class, AT_ID,
                    rowIndex);

...
void compute_mask(
    ...
    mask.setConstant(nelems, false);
    for (int idx_i = 0; idx_i < ids.size(); ++idx_i) {
        if (ids(idx_i) < nelems) {
            mask(ids(idx_i)) = true;
        }
    }
...
void bin_mat_from_bytes(
    ...
    auto bpl = (ncols >> 3u) + ((ncols & 7u) > 0u);
    UTILS_DIE_IF(payload_len != static_cast<size_t>(nrows * bpl), "Invalid payload_len in Eigen bin_mat_from_bytes!");

    bin_mat.setConstant(nrows, ncols, false);

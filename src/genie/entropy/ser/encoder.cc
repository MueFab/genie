...
genie::core::parameter::annotation::AlgorithmParameters SERParameters::convertToAlgorithmParameters() const {
    uint8_t n_pars = 1;
    ...
    for (auto i = 0; i < n_pars; ++i) {
        if (par_type.at(i) == core::DataType::BOOL) {
            par_val.push_back(genie::core::parameter::annotation::parameterToVector<uint8_t>(
                { values.at(i) }, par_type.at(i), par_num_array_dims.at(i), par_array_dims.at(i)));
        }
    }

    return genie::core::parameter::annotation::AlgorithmParameters(n_pars, par_ID, par_type,

...
void BSCEncoder::encode() {
    ...
    int ret =
        mpegg_bsc_compress(&destination, &destLen,
                            reinterpret_cast<const unsigned char *>(inputData.getdata().str().data()),
                            srcLen, lzpHashSize, lzpMinLen, blockSorter, coder);

    if (ret != 0) {
    std::cerr << "error with bsc compression\n";
    if (destination)
        free(destination);
    throw std::runtime_error("BSC compression failed");
    }
...
void BSCEncoder::encode(std::stringstream &input, std::stringstream &output) {
  ...
  int ret = mpegg_bsc_compress(&destination, &destLen, (const unsigned char *)input.str().c_str(),
                               srcLen, lzpHashSize, lzpMinLen, blockSorter, coder);
  if (ret != 0) {
    std::cerr << "error with bsc compression\n";
  }
...
void BSCEncoder::decode(std::stringstream &input, std::stringstream &output) {
  ...
  int ret = mpegg_bsc_decompress(&destination, &destLen, (const unsigned char *)input.str().c_str(),
                                 srcLen);
  if (ret != 0) {
...
BSCParameters::convertToAlgorithmParameters() const {
  uint8_t n_pars = 4;
  std::vector<uint8_t> par_ID{1, 2, 3, 4};
  const std::vector<genie::core::DataType> par_type{
      genie::core::DataType::UINT8, genie::core::DataType::UINT8, genie::core::DataType::UINT8,
      genie::core::DataType::UINT8};
  std::vector<uint8_t> par_num_array_dims(n_pars, 0);
  std::vector<uint8_t> values{MPEGG_BSC_DEFAULT_LZPHASHSIZE, MPEGG_BSC_DEFAULT_LZPMINLEN,
...
BSCParameters::compressorParameterSet(uint8_t compressor_ID) const {
  ...
  std::vector<genie::core::record::annotation_parameter_set::AlgorithmParameters>
  ...
  return genie::core::record::annotation_parameter_set::CompressorParameterSet(
      ...
      algorithm_parameters, n_in_vars, in_var_ID, prev_step_ID, prev_out_var_ID,
      n_completed_out_vars, completed_out_var_ID);
}

...
const AnalysisConfiguration& getCandidateConfig() {
    static const AnalysisConfiguration config = {
        ...
        {// Binarizations (unsigned)
         gabac::BinarizationId::BI, gabac::BinarizationId::TU, gabac::BinarizationId::EG, gabac::BinarizationId::TEG},
        {// Binarizations (signed)
         gabac::BinarizationId::SEG, gabac::BinarizationId::STEG},
        {// Binarization parameter (TEG and STEG only)
         1, 2, 3, 5, 7, 9, 15, 30, 255},
        {// Context modes
         gabac::ContextSelectionId::bypass, gabac::ContextSelectionId::adaptive_coding_order_0,
         gabac::ContextSelectionId::adaptive_coding_order_1, gabac::ContextSelectionId::adaptive_coding_order_2},
        {// LUT order
         0, 1},
        0,
        0};
...
void getOptimumOfBinarizationParameter(const AnalysisConfiguration& aconf, TraversalInfo* info) {
...
void getOptimumOfBinarization(const AnalysisConfiguration& aconf, TraversalInfo* info) {
    uint64_t min, max;
    int64_t smin, smax;
    if (aconf.maxValue == 0) {
...
void getOptimumOfDiffTransformedStream(const AnalysisConfiguration& aconf, TraversalInfo* info) {
    std::vector<gabac::BinarizationId> candidates =
        (!info->currConfig.transformedSequenceConfigurations[info->currStreamIndex].diffCodingEnabled)
...
void getOptimumOfLutTransformedStream(const AnalysisConfiguration& aconf, TraversalInfo* info) {
...
void getOptimumOfLutEnabled(const AnalysisConfiguration& aconf, TraversalInfo* info) {
    for (const auto& transID : aconf.candidateLutOrder) {
        ...
        info->numConfigs += 1;
        gabac::encode_cabac(gabac::BinarizationId::BI, {bits0}, gabac::ContextSelectionId::bypass,
        ...
        if (info->currConfig.transformedSequenceConfigurations[info->currStreamIndex].lutOrder > 0) {
            ...
            info->numConfigs += 1;
            gabac::encode_cabac(gabac::BinarizationId::BI, {bits1}, gabac::ContextSelectionId::bypass,
...
void getOptimumOfTransformedStream(const AnalysisConfiguration& aconf, TraversalInfo* info) {
...
void getOptimumOfSequenceTransform(const AnalysisConfiguration& aconf, TraversalInfo* info) {
    const std::vector<uint32_t> candidateDefaultParameters = {0};
    const std::vector<uint32_t>* params[] = {&candidateDefaultParameters, &candidateDefaultParameters,
    ...
    for (auto const& p : *(params[unsigned(info->currConfig.sequenceTransformationId)])) {
        ...
        gabac::getTransformation(info->currConfig.sequenceTransformationId).transform({p}, &info->stack.top().streams);
        ...
        for (unsigned i = 0; i < info->stack.top().streams.size(); ++i) {
            ...
            if (info->bestSequenceSize == std::numeric_limits<size_t>::max()) {
                info->ioconf->log(gabac::IOConfiguration::LogLevel::DEBUG)
                    << "Found no valid configuration for stream " << info->currStreamIndex << " of transformation"
                    << unsigned(info->currConfig.sequenceTransformationId) << " in word size "
                    << info->currConfig.wordSize << " Skipping!" << std::endl;
        ...
        if (info->currTotalSize < info->bestTotalSize) {
            info->ioconf->log(gabac::IOConfiguration::LogLevel::DEBUG)
                << "Found configuration compressing to " << info->currTotalSize << " bytes." << std::endl;
...
void getOptimumOfSymbolSequence(const AnalysisConfiguration& aconf, TraversalInfo* info) {
    for (const auto& transID : aconf.candidateSequenceTransformationIds) {
        ...
        info->currConfig.transformedSequenceConfigurations.resize(gabac::getTransformation(transID).wordsizes.size());
        // Core of analysis
    ...
    for (const auto& w : wordsize_cand) {
        ...
        if (info.stack.top().streams.front().getRawSize() % w != 0) {
            ioconf.log(gabac::IOConfiguration::LogLevel::WARNING)
                << "Input stream size " << info.stack.top().streams.front().getRawSize()
                << " is not a multiple of word size " << w << "! Skipping word size." << std::endl;
            continue;
        }

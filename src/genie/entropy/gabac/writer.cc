...
Writer::Writer(OBufferStream* const bitstream, const bool bypassFlag, const uint64_t numContexts)
...
void Writer::writeAsBIbypass(const uint64_t input, const std::vector<unsigned int>& binParams) {
    m_binaryArithmeticEncoder.encodeBinsEP(static_cast<unsigned int>(input), binParams[0]);
}

// ---------------------------------------------------------------------------------------------------------------------

void Writer::writeAsBIcabac(const uint64_t input, const std::vector<unsigned int>& binParams) {
    ...
    for (int i = cLength - 1; i >= 0; i--) {  // i must be signed
        const unsigned int bin = static_cast<unsigned int>(input >> static_cast<uint8_t>(i)) & 0x1u;
...
void Writer::writeAsTUbypass(const uint64_t input, const std::vector<unsigned int>& binParams) {
...
void Writer::writeAsTUcabac(const uint64_t input, const std::vector<unsigned int>& binParams) {
    const unsigned int cMax = binParams[0];
    const unsigned int cm = binParams[3];
    auto scan = m_contextModels.begin() + cm;
    for (uint64_t i = 0; i < input; i++) {
...
void Writer::writeAsEGbypass(const uint64_t input, const std::vector<unsigned int>&) {
    const auto valuePlus1 = static_cast<unsigned int>(input + 1);
...
void Writer::writeAsEGcabac(const uint64_t input, const std::vector<unsigned int>& binParams) {
    const auto valuePlus1 = static_cast<unsigned int>(input + 1);
    auto numLeadZeros = static_cast<unsigned int>(floor(log2(valuePlus1)));
...
void Writer::writeAsTEGbypass(const uint64_t input, const std::vector<unsigned int>& binParams) {
...
void Writer::writeAsTEGcabac(const uint64_t input, const std::vector<unsigned int>& binParams) {
    const unsigned int cTruncExpGolParam = binParams[0];
...
void Writer::writeAsSUTUbypass(const uint64_t input, const std::vector<unsigned int>& binParams) {
    ...
    for (i = 0, j = outputSymSize; i < outputSymSize; i += splitUnitSize) {
        ...
        const unsigned int val = input >> (j -= unitSize) & cMax;
...
void Writer::writeAsSUTUcabac(const uint64_t input, const std::vector<unsigned int>& binParams) {
    const unsigned int outputSymSize = binParams[0];
    const unsigned int splitUnitSize = binParams[1];
...
void Writer::writeLutSymbol(const uint64_t input, const uint8_t codingSubsymSize) {
...
void Writer::writeSignFlag(const int64_t input) {

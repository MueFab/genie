 ...
 * @details This file contains the definition of the `DecoderRegular` class,
 * which represents a descriptor parameter configuration for an LZMA-based
 * decoder. The `DecoderRegular` class implements the necessary interface to
 * read, configure, and write LZMA decoder settings used within the GENIE
 * framework.
...
constexpr uint8_t kModeLzma = 1;  //!< @brief LZMA mode identifier.

/**
 * @brief LZMA-specific parameter configuration for the decoder.
 * @details The `DecoderRegular` class is derived from
 * `core::parameter::desc_pres::DecoderRegular` and provides LZMA-specific
 * configurations for decoding descriptor subsequences. It stores configurations
 * for each subsequence and offers methods for setting, getting, and serializing
 *          these parameters.
 */
class DecoderRegular final : public core::parameter::desc_pres::DecoderRegular {
 ...
 public:
   ...
   * @return Const reference to the subsequence configuration at the specified
   * index.
   */
   ...
   * @brief Get the subsequence configuration at the specified index.
   * @param index Index of the configuration to retrieve.
   * @return Modifiable reference to the subsequence configuration at the
   * specified index.
   */
   ...
   * @param writer Reference to the BitWriter to store the serialized
   * configuration.
   */

...
void ContactMatrixParameters::Write(util::BitWriter& writer) const {
    ...
    for (const auto& sample_info : sample_infos_) {
        ...
        writer.WriteBits(static_cast<uint8_t>(sample_info.second.name.size()), 8);
    ...
    for (const auto& chr_info : chr_infos_) {
        ...
        writer.WriteBits(static_cast<uint8_t>(chr_info.second.name.size()), 8);
    ...
    for (const auto& method_info : norm_method_infos_) {
        ...
        writer.WriteBits(static_cast<uint8_t>(method_info.second.name.size()), 8);
        writer.Write(method_info.second.name);
        // 7 reserved bits merged into the next statement
        writer.WriteBits(static_cast<uint8_t>(method_info.second.mult_flag), 8);
    }
    ...
    for (const auto& mat_info : norm_mat_infos_) {
        ...
        writer.WriteBits(static_cast<uint8_t>(mat_info.second.name.size()), 8);

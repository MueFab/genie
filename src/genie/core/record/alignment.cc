...
Alignment::Alignment(std::string &&_ecigar_string, const uint8_t _reverse_comp)
...
void Alignment::addMappingScore(const int32_t score) { mapping_score.push_back(score); }
...
void Alignment::write(util::BitWriter &writer) const {
    ...
    for (const auto s : mapping_score) {

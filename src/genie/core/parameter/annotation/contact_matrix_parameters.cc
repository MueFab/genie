...
void ContactMatrixParameters::read(util::BitReader& reader) {
    ...
    for (auto idx_i = 0; idx_i < num_samples; ++idx_i) {
    ...
    for (auto idx_i = 0; idx_i < num_chrs; ++idx_i) {
    ...
    for (auto idx_i = 0; idx_i < num_bin_size_multipliers; ++idx_i)
    ...
    for (auto idx_i = 0; idx_i < num_norm_methods; ++idx_i) {
    ...
    for (auto idx_i = 0; idx_i < num_norm_matrices; ++idx_i) {
...
void ContactMatrixParameters::write(core::Writer& writer) const {
    ...
    for (auto idx_i = 0; idx_i < num_samples; ++idx_i) {
        writer.Write(sample_ID[idx_i], 8);
        writer.Write(sample_name_len[idx_i], 8);
        for (auto byte : sample_name[idx_i]) writer.Write(byte, 8);
    }
    writer.Write(num_chrs, 8);
    for (auto idx_i = 0; idx_i < num_chrs; ++idx_i) {
        writer.Write(chr_ID[idx_i], 8);
        writer.Write(chr_name_len[idx_i], 8);
        for (auto byte : chr_name[idx_i]) writer.Write(byte, 8);
        writer.Write(chr_length[idx_i], 64);
    }
    writer.Write(bin_size, 32);
    writer.Write(tile_size, 32);
    writer.Write(num_bin_size_multipliers, 8);
    for (auto idx_i = 0; idx_i < num_bin_size_multipliers; ++idx_i)
      writer.Write(bin_size_multiplier[idx_i], 32);

    writer.Write(num_norm_methods, 8);
    for (auto idx_i = 0; idx_i < num_norm_methods; ++idx_i) {
        writer.Write(norm_method_ID[idx_i], 8);
        writer.Write(norm_method_name_len[idx_i], 8);
        for (auto byte : norm_method_name[idx_i]) writer.Write(byte, 8);
        writer.WriteReserved(7);
        writer.Write(norm_method_mult_flag[idx_i], 1);
    }
    writer.Write(num_norm_matrices, 8);
    for (auto idx_i = 0; idx_i < num_norm_matrices; ++idx_i) {
        writer.Write(norm_matrix_ID[idx_i], 8);
        writer.Write(norm_matrix_name_len[idx_i], 8);
        for (auto byte : norm_matrix_name[idx_i]) writer.Write(byte, 8);
    }
}
...
void ContactMatrixParameters::write(util::BitWriter& writer) const {
  ...
  for (auto idx_i = 0; idx_i < num_samples; ++idx_i) {
    writer.WriteBits(sample_ID[idx_i], 8);
    for (auto byte : sample_name[idx_i]) {
  ...
  for (auto idx_i = 0; idx_i < num_chrs; ++idx_i) {
    writer.WriteBits(chr_ID[idx_i], 8);
    for (auto byte : chr_name[idx_i]) {
      writer.WriteBits(byte, 8);
    }
    writer.WriteReserved(8);
    writer.WriteBits(chr_length[idx_i], 64);
  }
  writer.WriteBits(bin_size, 32);
  writer.WriteBits(tile_size, 32);
  writer.WriteBits(num_bin_size_multipliers, 8);
  for (auto idx_i = 0; idx_i < num_bin_size_multipliers; ++idx_i)
    writer.WriteBits(bin_size_multiplier[idx_i], 32);

  writer.WriteBits(num_norm_methods, 8);
  for (auto idx_i = 0; idx_i < num_norm_methods; ++idx_i) {
    writer.WriteBits(norm_method_ID[idx_i], 8);
    for (auto byte : norm_method_name[idx_i]) writer.WriteBits(byte, 8);
    writer.WriteReserved(8);
    writer.WriteBits(norm_method_mult_flag[idx_i], 1);
    writer.WriteReserved(7);
  }
  writer.WriteBits(num_norm_matrices, 8);
  for (auto idx_i = 0; idx_i < num_norm_matrices; ++idx_i) {
    writer.WriteBits(norm_matrix_ID[idx_i], 8);
    for (auto byte : norm_matrix_name[idx_i]) writer.WriteBits(byte, 8);
    writer.WriteReserved(8);
  }
}

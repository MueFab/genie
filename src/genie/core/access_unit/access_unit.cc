...
void AccessUnit::Descriptor::Write(util::BitWriter& writer) const {
  ...
  for (size_t idx_i = 0; idx_i < subdesc_.size(); ++idx_i) {
    if (idx_i < subdesc_.size() - 1) {
      writer.WriteBits(subdesc_[idx_i].GetRawSize(), 32);
    }
    subdesc_[idx_i].Write(writer);
  }
}
...
AccessUnit::Descriptor::Descriptor(GenDesc id, const size_t count,
  ...
  for (size_t idx_i = 0; idx_i < count; ++idx_i) {
    ...
    if (s) {
      subdesc_.emplace_back(GenSubIndex{id, static_cast<uint16_t>(idx_i)}, s,
                            reader);
    } else {
      subdesc_.emplace_back(GenSubIndex{id, static_cast<uint16_t>(idx_i)},
                            util::DataBlock(0, 4));
    }
  }
}

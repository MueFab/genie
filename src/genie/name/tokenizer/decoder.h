 ...
 *
 * @brief Declaration of the Decoder class for handling token-based name decoding in the GENIE framework.
 *
 * This file contains the declaration of the `Decoder` class, which is a specialized implementation
 * of the `NameDecoder` interface. The `Decoder` class is responsible for decoding encoded name tokens
 * from genomic records back into readable strings using the GENIE framework's tokenizer.
 *
 * @details The `Decoder` class provides a token-based approach to name decoding, reconstructing
 * genomic record names from token sequences that were previously encoded. It is designed to work
 * within the `tokenizer` module of the GENIE library.
 */
 ...
 *
 * The `Decoder` class is responsible for decoding the names of genomic records using
 * a token-based approach. It processes encoded `AccessUnit::Descriptor` objects, extracts
 * the name tokens, and reconstructs the original record names. The class is designed to work
 * within the `tokenizer` module of the GENIE library.
 */
class Decoder : public core::NameDecoder {
     ...
     *
     * This static method takes a vector of `SingleToken` objects and combines them to form
     * a complete readable string, reconstructing the original name from its tokenized representation.
     *
     * @param rec The vector of `SingleToken` objects representing the encoded name.

...
#include <read_coding/spring/params.h>
#include <read_coding/spring/spring.h>
#include <chrono>
...
#include <read_coding/spring/generate-read-streams.h>
namespace genie {
...
void SpringEncoder::preprocessIteration(backbone::mpegg_rec::MpeggChunk &&data) {
    backbone::mpegg_rec::MpeggChunk t = std::move(data);
    for (int j = 0; j < 2; j++) {
        if (j == 1 && !cp.paired_end) continue;
...
void SpringEncoder::flowIn(backbone::mpegg_rec::MpeggChunk &&t, size_t id) {
    if (!id) {

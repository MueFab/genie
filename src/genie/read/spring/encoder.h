 ...
 *
 * This class inherits from `genie::core::ReadEncoder` and is designed to manage the
 * process of encoding read sequences into access units. The Encoder class utilizes
 * the Preprocessor class to preprocess input records and generates the final encoded
 * access units that can be stored or further processed.
 */
class Encoder : public genie::core::ReadEncoder {
 private:
    Preprocessor preprocessor;  //!< @brief Handles preprocessing of input records and file management.
 ...
 public:
     ...
     *
     * This function is called to feed chunks of records into the encoding pipeline.
     * It utilizes the preprocessor to handle the records and manages synchronization
     * using the `util::Section` parameter.
     *

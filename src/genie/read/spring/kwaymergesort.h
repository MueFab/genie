...
public:
  ...
  // constructor
  MERGE_DATA(const string &_data,
             istream *_stream,
             bool(*_compFunc)(const string &a, const string &b))
    :
    data(_data),
    stream(_stream),
    compFunc(_compFunc) {}
...
public:
  ...
  KwayMergeSort(const string &inFile,
                ostream *out = nullptr,
                bool(*compareFunction)(const string &a, const string &b) = NULL,
...
public:
  ...
  KwayMergeSortIterator() :
    _owner(nullptr),
    _eof(false) {};
  KwayMergeSortIterator(KwayMergeSort * owner,
                        string elem,
                        bool eof = false) :
    _owner(owner),
    _elem(elem),
...
// constructor
KwayMergeSort::KwayMergeSort(const string &inFile,
                                ostream *out,
...
void KwayMergeSort::InitializeMergeStep() {
  ...
  for (size_t i = 0; i < _vTempFiles.size(); ++i) {
    ...
    _outQueue.insert
    (MERGE_DATA(_line, _vTempFiles[i], _compareFunction));
  }
}
...
string KwayMergeSort::MergeStepByStep() {
  ...
  if (*(lowest.stream))
    _outQueue.insert
    (MERGE_DATA(_line, lowest.stream, _compareFunction));
...
KwayMergeSortIterator KwayMergeSort::end() {
  return KwayMergeSortIterator(this, {}, true);
}

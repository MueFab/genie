...
void decode_streams(core::AccessUnit& au, bool paired_end, bool combine_pairs,
    ...
    while (!au.get(core::gen_sub::kRtype).end()) {
        ...
        if (rtype == 5) {
            ...
            for (uint32_t idx_i = 0; idx_i < rlen; idx_i++) {
                refBuf.push_back(
            ...
            uint32_t rlen[2];
            for (int idx_i = 0; idx_i < number_of_record_segments; idx_i++)
                rlen[idx_i] = (uint32_t)(au.get(core::gen_sub::kReadLength).pull()) + 1;      // rlen
            ...
            bool reverseComp[2];
            for (int idx_i = 0; idx_i < number_of_record_segments; idx_i++)
                reverseComp[idx_i] = au.get(core::gen_sub::kReverseComplement).pull();  // rcomp
            if (rtype == 3) {
                // type M
                for (int idx_i = 0; idx_i < number_of_record_segments; idx_i++) {
                    ...
                    while (true) {
                        ...
                        if (mmtype_0 != 0)  // idx_i.e., not substitution
                        ...
                        const auto mmtype_1 = au.get(core::gen_sub::kMismatchTypeSubstBase).getMismatchDecoder()->dataLeft()
                                                  ? au.get(core::gen_sub::kMismatchTypeSubstBase)
                                                        ...
                                                        ->decodeMismatch(GetAlphabetProperties(core::AlphabetId::kAcgtn)
                                                                             .inverseLut[cur_read[idx_i][abs_mmpos]])
                                                  : GetAlphabetProperties(core::AlphabetId::kAcgtn).inverseLut['N'];
                        cur_read[idx_i][abs_mmpos] = GetAlphabetProperties(core::AlphabetId::kAcgtn).lut[mmtype_1];
                        abs_mmpos++;
                    }
                }
            }
            ...
            for (int idx_i = 0; idx_i < number_of_record_segments; idx_i++) {
                if (reverseComp[idx_i] == 1) cur_read[idx_i] = reverse_complement(cur_read[idx_i], rlen[idx_i]);
            }
            bool first_read_flag = true;
            for (int idx_i = 0; idx_i < number_of_record_segments; idx_i++) {
                if (paired_end) {
                    if ((idx_i == 0 && read_1_first) || (idx_i == 1 && !read_1_first)) {
                        first_read_flag = true;
                    } else {
                        first_read_flag = false;
                    }
                }
                cur_record.seq = cur_read[idx_i];
                if (!names.empty()) {
    ...
    if (paired_end && combine_pairs) {
        ...
        if (size_unmatched > 0) {
            ...
            for (size_t idx_i = 0; idx_i < size_unmatched; idx_i++)
                record_index_for_sorting[idx_i] =
                    std::make_pair(pos_in_unmatched_same_au_0[unmatched_same_au[1][idx_i].name], static_cast<uint32_t>(idx_i));
            std::sort(
            ...
            for (size_t idx_i = 0; idx_i < size_unmatched; idx_i++)
                matched_records[1].push_back(unmatched_same_au[1][record_index_for_sorting[idx_i].second]);
        }
    }
    return;
}
...
void Decoder::flowIn(genie::core::AccessUnit&& t, const util::Section& id) {
    ...
    for (size_t idx_i = 0; idx_i < matched_records[0].size(); ++idx_i) {
        chunk.getData().emplace_back(cp.paired_end ? (uint8_t)2 : (uint8_t)1, core::record::ClassType::CLASS_U,
                                     std::move(matched_records[0][idx_i].name), "", (uint8_t)0);
        core::record::Segment seg(std::move(matched_records[0][idx_i].seq));
        if (!matched_records[0][idx_i].qv.empty()) {
            seg.addQualities(std::move(matched_records[0][idx_i].qv));
        }
        chunk.getData().back().addSegment(std::move(seg));
        if (cp.paired_end) {
            core::record::Segment seg2(std::move(matched_records[1][idx_i].seq));
            if (!matched_records[1][idx_i].qv.empty()) {
                seg2.addQualities(std::move(matched_records[1][idx_i].qv));
            }
    ...
    if (!combine_pairs) {
        for (size_t idx_i = 0; idx_i < unmatched_records[0].size(); ++idx_i) {
            chunk.getData().emplace_back(cp.paired_end ? (uint8_t)2 : (uint8_t)1, core::record::ClassType::CLASS_U,
                                         std::move(unmatched_records[0][idx_i].name), "", (uint8_t)0, true);
            // last parameter is read_1_first
            core::record::Segment seg(std::move(unmatched_records[0][idx_i].seq));
            if (!unmatched_records[0][idx_i].qv.empty()) {
                seg.addQualities(std::move(unmatched_records[0][idx_i].qv));
            }
            chunk.getData().back().addSegment(std::move(seg));
        }
        for (size_t idx_i = 0; idx_i < unmatched_records[1].size(); ++idx_i) {
            chunk.getData().emplace_back(cp.paired_end ? (uint8_t)2 : (uint8_t)1, core::record::ClassType::CLASS_U,
                                         std::move(unmatched_records[1][idx_i].name), "", (uint8_t)0, false);
            // last parameter is read_1_first
            core::record::Segment seg(std::move(unmatched_records[1][idx_i].seq));
            if (!unmatched_records[1][idx_i].qv.empty()) {
                seg.addQualities(std::move(unmatched_records[1][idx_i].qv));
            }
            chunk.getData().back().addSegment(std::move(seg));
        }
    }
    {
        ...
        if (cp.paired_end && combine_pairs) {
            for (int idx_j = 0; idx_j < 2; idx_j++) {
                std::ostream& tmpout = (idx_j == 0) ? fout_unmatched1 : fout_unmatched2;
                std::ostream& tmpoutreadnames = (idx_j == 0) ? fout_unmatched_readnames_1 : fout_unmatched_readnames_2;
                for (auto& fastqRecord : unmatched_records[idx_j]) {
                    tmpout << fastqRecord.name << "\n";
                    tmpout << fastqRecord.seq << "\n";
                    tmpout << fastqRecord.qv << "\n";
                    tmpoutreadnames << fastqRecord.name << "\t" << unmatched_record_index[idx_j]++ << "\n";
                }
            }
        }
    }
...
void Decoder::readRec(std::ifstream& idx_i, Record& r) {
    UTILS_DIE_IF(!std::getline(idx_i, r.name), "Error reading tmp file");
    UTILS_DIE_IF(!std::getline(idx_i, r.seq), "Error reading tmp file");
    UTILS_DIE_IF(!std::getline(idx_i, r.qv), "Error reading tmp file");
}
...
void Decoder::FlushIn(uint64_t& pos) {
    ...
    if (!cp.ureads_flag && cp.paired_end && combine_pairs) {
        ...
        if (size_unmatched > 0) {
            ...
            // now build an index that tells for read idx_i in file_2, where it's pair in
            // file_1 lies.
            ...
            for (uint32_t idx_i = 0; idx_i < size_unmatched; idx_i++) {
                reverse_index_file_2[idx_i] = index_file_1[reverse_index_file_2[idx_i]];
            }
            ...
            for (uint32_t idx_i = 0; idx_i <= size_unmatched / bin_size; idx_i++) {
                uint32_t num_records_bin = bin_size;
                if (idx_i == size_unmatched / bin_size) num_records_bin = size_unmatched % bin_size;
                if (num_records_bin == 0) break;  // we are done
                std::ifstream fin_unmatched2(file_unmatched_fastq2);
                uint32_t bin_start = idx_i * bin_size;
                for (uint32_t idx_j = 0; idx_j < size_unmatched; idx_j++) {
                    if (reverse_index_file_2[idx_j] >= bin_start && reverse_index_file_2[idx_j] < bin_start + num_records_bin) {
                        readRec(fin_unmatched2, records_bin[reverse_index_file_2[idx_j] - bin_start]);
                    } else {
                        readRec(fin_unmatched2, tmpFastqRecord);
                    }
                }
                fin_unmatched2.close();
                for (uint32_t idx_j = 0; idx_j < num_records_bin; idx_j++) {
                    ...
                    core::record::Segment s2(std::move(records_bin[idx_j].seq));
                    if (!records_bin[idx_j].qv.empty()) {
                        s2.addQualities(std::move(records_bin[idx_j].qv));
                    }

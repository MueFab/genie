...
void SpringEncoder::flushIn(size_t& pos) {
    ...
    generate_read_streams(preprocessor.temp_dir, preprocessor.cp, entropycoder, params);
    ...
    if (preprocessor.cp.preserve_quality || preprocessor.cp.preserve_id) {
        ...
        reorder_compress_quality_id(preprocessor.temp_dir, preprocessor.cp, qvcoder, namecoder, entropycoder, params);

...
void reverse_complement(const char *s, char *s1, const int readlen) {
    for (int j = 0; j < readlen; j++) s1[j] = chartorevchar[static_cast<uint8_t>(s[readlen - j - 1])];
    s1[readlen] = '\0';
}
...
void write_dna_in_bits(const std::string &read, std::ofstream &fout) {
    ...
    dna2int[static_cast<uint8_t>('C')] = 2;  // chosen to align with the bitset representation
...
void read_dna_from_bits(std::string &read, std::ifstream &fin) {
    ...
    read.resize(readlen);
    const uint16_t num_bytes_to_read = (static_cast<uint32_t>(readlen) + 4 - 1) / 4;
...
void read_dnaN_from_bits(std::string &read, std::ifstream &fin) {
    ...
    fin.read(reinterpret_cast<char *>(&readlen), sizeof(uint16_t));
    read.resize(readlen);
    const uint16_t num_bytes_to_read = (static_cast<uint32_t>(readlen) + 2 - 1) / 2;
    fin.read(reinterpret_cast<char *>(&bitarray[0]), num_bytes_to_read);
    uint8_t pos_in_bitarray = 0;
    for (int i = 0; i < readlen / 2; i++) {
        for (int j = 0; j < 2; j++) {

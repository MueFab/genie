...
void binarize_row_bin(Int8MatDtype& allele_mat,
...
void sort_bin_mat(BinMatDtype& bin_mat, UIntVecDtype& row_ids,
                  UIntVecDtype& col_ids, SortingAlgoID sort_row_method,
...
[[maybe_unused]] void entropy_encode_bin_mat(BinMatDtype& bin_mat,
  ...
  if (codec_ID == genie::core::AlgoID::JBIG) {
    mpegg_jbig_compress_default(&compressed_data, &compressed_data_len,
  ...
  payload = std::vector<uint8_t>(compressed_data,
...
void encode_genotype(
    ...
    // Options
    size_t block_size, BinarizationID binarization_ID, ConcatAxis concat_axis,
    bool transpose_mat, SortingAlgoID sort_row_method,
    SortingAlgoID sort_col_method, genie::core::AlgoID codec_ID) {
  UTILS_DIE_IF(binarization_ID == BinarizationID::UNDEFINED,
               "Invalid BinarizationID");
  UTILS_DIE_IF(sort_row_method == SortingAlgoID::UNDEFINED,
               "Invalid SortingAlgoID");
  UTILS_DIE_IF(sort_col_method == SortingAlgoID::UNDEFINED,
               "Invalid SortingAlgoID");

  //    params.SetBinarizationID(binarization_ID);
  //    params.SetConcatAxis(concat_axis);
  //    params.SetTransposeAllelesMatFlag(transpose_mat);
  //    params.SetSortAllelesRowsFlag(sort_row_method !=
  //    SortingAlgoID::NO_SORTING);
  //    params.SetSortAllelesColsFlag(sort_col_method !=
  //    SortingAlgoID::NO_SORTING);

  auto tmp_params = GenotypeParameters(
      binarization_ID, concat_axis, transpose_mat,
      sort_row_method != SortingAlgoID::NO_SORTING,
      sort_col_method != SortingAlgoID::NO_SORTING, codec_ID,
      true,  // Encode Phase data
      transpose_mat, sort_row_method != SortingAlgoID::NO_SORTING,
      sort_col_method != SortingAlgoID::NO_SORTING, codec_ID);
...

  {
    ...
    binarize_allele_mat(allele_mat, allele_bin_mat_vect, num_bin_mats, amax_vec,
  ...
  for (auto i_mat = 0u; i_mat < num_bin_mats; i_mat++) {
    sort_bin_mat(allele_bin_mat_vect[i_mat], allele_row_ids_vect[i_mat],
                 allele_col_ids_vect[i_mat], sort_row_method, sort_col_method);
  }

  {
    ...
    sort_bin_mat(phasing_mat, phasing_row_ids, phasing_col_ids, sort_row_method,
                 sort_col_method);
  }

  params = std::move(tmp_params);
  //    sort_format(recs, opt.block_size < recs.size() ? opt.block_size :
  //    recs.size(), block);
}

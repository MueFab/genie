...
namespace lae {
FullLocalAssemblyDecoder::FullLocalAssemblyDecoder(std::unique_ptr<StreamContainer> container, uint32_t cr_buf_max_size,
                                                   bool _debug)
    : mapping_pos(0), state(nullptr), lrd(std::move(container)), debug(_debug) {
    init(cr_buf_max_size);
}

FullLocalAssemblyDecoder::~FullLocalAssemblyDecoder() { local_assembly_state_destroy(state); }
...
void FullLocalAssemblyDecoder::decodePair(util::SamRecord *mpegg_record_segment_1,
    ...
    local_assembly_state_add_read(state, mpegg_record_segment_1->seq.c_str(), mpegg_record_segment_1->cigar.c_str(),
                                  mpegg_record_segment_1->pos);
    local_assembly_state_add_read(state, mpegg_record_segment_2->seq.c_str(), mpegg_record_segment_2->cigar.c_str(),
                                  mpegg_record_segment_2->pos);

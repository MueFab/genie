...
void pack(const ProgramOptions& programOptions, std::unique_ptr<StreamContainer> container, uint32_t read_length,
    ...
    for (size_t descriptor = 0; descriptor < format::NUM_DESCRIPTORS; ++descriptor) {
        for (size_t subdescriptor = 0;
             subdescriptor < format::getDescriptorProperties()[descriptor].number_subsequences; ++subdescriptor) {
    ...
    ParameterSet ps = createQuickParameterSet(PARAMETER_SET_ID, READ_LENGTH, paired, QV_PRESENT,
                                              DataUnit::DatasetType::ALIGNED, configs);
    ...
    AccessUnit au = createQuickAccessUnit(ACCESS_UNIT_ID, PARAMETER_SET_ID, readNum, DataUnit::AuType::U_TYPE_AU,

...
void LocalAssemblyReadEncoder::addPair(const util::SamRecord& rec1, const std::string& ref1,
                                       const util::SamRecord& rec2, const std::string& ref2) {
            ...
            case 'X':
                for (size_t i = 0; i < count; ++i) {
                    ...
                    if (read[read_pos] != ref[ref_offset]) {
                        if (ref[ref_offset] == 0) {
                            clips[rightClip] += format::getAlphabetProperties(format::ParameterSet::AlphabetID::ACGTN)
                            ...
                            container->mmtype_1.push_back(
                                format::getAlphabetProperties(format::ParameterSet::AlphabetID::ACGTN)
                                    .inverseLut[read[read_pos]]);
                            rightClip = true;
                        }
                    } else {

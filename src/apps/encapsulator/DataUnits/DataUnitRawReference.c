...
DataUnitRawReference* initDataUnitRawReference(){
    ...
    if(
        dataUnitRawReference->sequence_ID == NULL ||
        dataUnitRawReference->sequence_start == NULL ||
        dataUnitRawReference->sequence_end == NULL ||
        dataUnitRawReference->ref_sequence == NULL
    ){
...
void addRawSequence(
        DataUnitRawReference* dataUnitRawReference,
        uint16_t sequenceId,
...
DataUnitRawReference* parseRawReference(char* path){
    ...
    for(uint16_t sequence_i = 0; sequence_i < sequenceCount; sequence_i++){
        ...
        if(
                1 != fread(&sequenceIdBE, sizeof(uint16_t), 1, inputFile)
        ){
        ...
        uint64_t startPos = 0;
        if(
            1 != fread(((uint8_t*)&startPos)+4, sizeof(uint8_t), 1, inputFile)
        ...
        uint64_t endPos = 0;
        if(
                1 != fread(((uint8_t*)&endPos)+4, sizeof(uint8_t), 1, inputFile)
...
int writeRawReference(DataUnitRawReference* dataUnitRawReferenceToWrite, char* path){
    ...
    for(uint16_t sequence_i = 0; sequence_i < sequenceCount; sequence_i++){
        ...
        if(
                1 != fwrite(&sequenceIdBE, sizeof(sequenceIdBE), 1, outputFile)
        ){
        ...
        if(
            5 != fwrite(((uint8_t*)&startPosBE)+3, sizeof(uint8_t), 5, outputFile)
        ){
        ...
        if(
            5 != fwrite(((uint8_t*)&endPosBE)+3, sizeof(uint8_t), 5, outputFile)
        ){
            fprintf(stderr, "Could not write start position.\n");
            return -1;
        }
        ...
        if(
                sequenceSize != fwrite(
                    dataUnitRawReferenceToWrite->ref_sequence[sequence_i],
                    sizeof(char),
                    sequenceSize,
                    outputFile
                )

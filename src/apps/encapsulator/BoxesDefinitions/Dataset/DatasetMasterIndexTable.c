...
bool getAccessUnitByteOffsetFromSequence(
        Sequence_indexation *sequence_index,
        uint16_t classId,
        uint32_t auId,
        uint64_t *byteOffset
);
...
void initIndexUnaligned(DatasetMasterIndexTable* datasetMasterIndexTable){
    ...
    datasetMasterIndexTable->uAccessUnitsIndex.ref_sequence_id = (uint16_t*)calloc(numberUAccessUnits,sizeof(uint16_t));
    datasetMasterIndexTable->uAccessUnitsIndex.ref_start_position = (uint64_t*)calloc(numberUAccessUnits,sizeof(uint64_t));
    datasetMasterIndexTable->uAccessUnitsIndex.ref_end_position = (uint64_t*)calloc(numberUAccessUnits,sizeof(uint64_t));
    ...
    if (datasetMasterIndexTable->uAccessUnitsIndex.byteOffset != NULL) {
        ...
        datasetMasterIndexTable->uAccessUnitsIndex.offsetsDescriptorUnmappedTree = (Tree**)calloc(
                numberDescriptorsUnmapped, sizeof(Tree*)
        );
...
DatasetMasterIndexTable* initDatasetMasterIndexTable(DatasetContainer *datasetContainer){
    ...
    datasetMasterIndexTable->sequence_indexes.sequence_indexes = (Sequence_indexation *)malloc(sequences_count*sizeof(Sequence_indexation));
    ...
    for(uint16_t seq_i=0; seq_i<sequences_count; seq_i++){
        ...
        for(uint8_t classId = 0; classId<classesCountForAligned; classId++){
            ...
            datasetMasterIndexTable->sequence_indexes.sequence_indexes[seq_i].classIndex[classId].number_AUs=numberAUs;
            ...
            if(!isBlockHeaderFlagSet(getDatasetHeader(datasetContainer))) {
                ...
                for (typeof(numberAUs) auId = 0; auId < numberAUs; auId++) {
                    datasetMasterIndexTable->sequence_indexes.sequence_indexes[seq_i].classIndex[classId].au_indexings[auId]
...
bool writeUnalignedIndexing(DatasetMasterIndexTable* datasetMasterIndexTable, OutputBitstream* outputBitstream){
    ...
    for(uint32_t uAccessUnit_i=0; uAccessUnit_i<datasetMasterIndexTable->uAccessUnitsIndex.numberUAccessUnits; uAccessUnit_i++) {
        uint32_t uAccessUnitOffset =
                (uint32_t) datasetMasterIndexTable->uAccessUnitsIndex.AU_byte_offset[uAccessUnit_i];
...
uint64_t getSizeContentUnalignedDatasetMasterIndexTable(DatasetMasterIndexTable* datasetMasterIndexTable){
    ...
    for(uint32_t uAccessUnit_i=0; uAccessUnit_i<datasetMasterIndexTable->uAccessUnitsIndex.numberUAccessUnits; uAccessUnit_i++) {
        contentSize += bitsToBytes(
            isByteOffsetUint64(datasetHeader) ? 64 : 32
        ); //au_byte_offset
        ...
        if (!isBlockHeaderFlagSet(getDatasetHeader(datasetMasterIndexTable->datasetContainer))) {
            ...
            contentSize += (uint64_t)(numberDescriptorsUnmapped * (uint16_t)(isByteOffsetUint64(datasetHeader)?8:4));
        }
    }
    return contentSize;
}
...
uint64_t getAUStartPosition(
    ...
    return datasetMasterIndexTable->sequence_indexes
                   .sequence_indexes[sequenceIndex]
                   .classIndex[classId]
                   .au_indexings[AU_id]
                   .au_start_position;
}
...
void setAUOffset(DatasetMasterIndexTable *datasetMasterIndexTable,
    ...
    datasetMasterIndexTable->
        sequence_indexes.sequence_indexes[sequenceIndex]
        .classIndex[classIndex]
        .au_indexings[AU_id]
...
void setStartAndEnd(DatasetMasterIndexTable *datasetMasterIndexTable,
    ...
    if(sequenceIndex<datasetMasterIndexTable->sequence_indexes.number_sequences){
        ...
        if(classIndex <datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequenceIndex].number_of_classes){
            if(AU_id<datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequenceIndex].classIndex[classIndex].number_AUs){
                datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequenceIndex].classIndex[classIndex]
...
void setOffset(DatasetMasterIndexTable *datasetMasterIndexTable,
    ...
    if(sequence_i<datasetMasterIndexTable->sequence_indexes.number_sequences){
        if(class_i<datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequence_i].number_of_classes){
            if(AU_i<datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequence_i].classIndex[class_i].number_AUs){
                if(descriptor_i< datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequence_i]
                                         .classIndex[class_i].au_indexings[AU_i].block_byte_offset.descriptors_number
                ) {
                    datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequence_i].classIndex[class_i].
                        au_indexings[AU_i].block_byte_offset.block_byte_offset[descriptor_i] = offset;
                    if(!isBlockHeaderFlagSet(getDatasetHeader(datasetMasterIndexTable->datasetContainer))) {
...
void insertFinalOffset(DatasetMasterIndexTable *datasetMasterIndexTable, ClassType classType, uint16_t classIndex,
    ...
    if(classType.classType != CLASS_TYPE_CLASS_U) {
        ...
        if (!insert(
                datasetMasterIndexTable->uAccessUnitsIndex.offsetsDescriptorUnmappedTree[descriptorIndex],
                treeNode
        )) {
            free(treeNode);
        }
    ...
    for(uint16_t seq_i = 0; seq_i<datasetMasterIndexTable->sequence_indexes.number_sequences; seq_i++){
        ...
        for(uint16_t class_i = 0; class_i<numberClassesForAligned; class_i++){
            ...
            for(uint32_t au_id = 0; au_id<numberAUs; au_id++){
                AU_indexing* au_position =
                        &(datasetMasterIndexTable->sequence_indexes.sequence_indexes[seq_i].classIndex[class_i].au_indexings[au_id]);

                if(isByteOffsetUint64(getDatasetHeader(datasetContainer))){
                    readNBitsBigToNativeEndian64(
                            &inputBitstream,
                            64,
                            &(au_position->AU_byte_offset)
                    );
                }else{
                    uint32_t au_offset;
                    readNBitsBigToNativeEndian32(
                            &inputBitstream,
                            32,
                            &au_offset
                    );
                ...
                if(isPosOffsetUint40(getDatasetHeader(datasetContainer))) {
                    readNBitsBigToNativeEndian64(
                            &inputBitstream,
                            40,
                            &(au_position->au_start_position)
                    );
                    readNBitsBigToNativeEndian64(
                            &inputBitstream,
                            40,
                            &(au_position->au_end_position)
                    );
                }else{
                ...
                if(getDatasetType(getDatasetHeader(datasetContainer)) == 2){
                    ...
                    if(isPosOffsetUint40(getDatasetHeader(datasetContainer))){
                        readNBitsBigToNativeEndian64(
                                &inputBitstream, 40, &ref_start_position
                        );
                        readNBitsBigToNativeEndian64(
                                &inputBitstream, 40, &ref_end_position
                        );
                    }else{
                        readNBitsBigToNativeEndian64(
                                &inputBitstream, 32, &ref_start_position
                        );
                        readNBitsBigToNativeEndian64(
                                &inputBitstream, 32, &ref_end_position
                        );
                    }
                ...
                if(isMultipleAlignment(getDatasetHeader(datasetContainer))){
                    if(isPosOffsetUint40(getDatasetHeader(datasetContainer))){
                        readNBitsBigToNativeEndian64(&inputBitstream, 40,
                        ...
                        readNBitsBigToNativeEndian32(&inputBitstream, 32,
                                                     &extended_au_start_position);
...
bool getBlockByteOffsetFromSequence(Sequence_indexation *sequenceIndex, ClassTypeEnum classId, uint32_t auId, uint32_t descriptorId,
                               uint64_t *byteOffset) {

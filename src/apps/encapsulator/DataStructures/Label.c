...
bool writeRegionsForDataset(RegionsForDataset *regionsForDataset, OutputBitstream *outputFile){
    bool datasetidSuccessfulWrite = writeNBitsShiftAndConvertToBigEndian16(
            outputFile,
            16,
            regionsForDataset->datasetId
    );
    uint8_t numRegions = (uint8_t) getSize(regionsForDataset->regions);
    bool numRegionsSuccessfulWrite = writeToBitstream(
            outputFile,
            numRegions
    );
...
RegionsForDataset* parseRegionsForDataset(InputBitstream *inputFile){
    ...
    bool datasetIdSuccesfulRead = readNBitsBigToNativeEndian16(
            inputFile,
            16,
            &datasetId
    );
    bool numRegionsSuccesfulRead = readNBits8(
            inputFile,
            8,
            &numRegions
    );
...
bool writeContentLabel(Label *label, FILE *outputFile){
    ...
    bool numberRegionsSuccessfulWrite = writeNBitsShiftAndConvertToBigEndian16(
            &outputBitstream,
            16,
            numberRegionsForDataset
    );
...
Label* parseLabel(FILE* inputFile){
    ...
    bool numberRegionsForDatasetSuccessfulRead = readNBitsBigToNativeEndian16(
            &inputBitstream, 16, &numberRegionsForDataset
    );
...
bool writeRegion(Region *region, OutputBitstream *outputFile){
    bool sequenceIDSuccessfulWrite = writeNBitsShiftAndConvertToBigEndian16(
            outputFile, 16, region->sequence_Id
    );
    uint8_t number_classes = (uint8_t) getNumberClasses(region);
    bool numberClassesSuccessfulWrite = writeNBitsShift(
            outputFile,
            4,
            (char*)&number_classes
    );
    ...
    for(
        ...
        if (checkClass(region->classes, class_i)){
            if (
                    !writeNBitsShift(outputFile, 4, (char*)&class_i)
            ) {
    ...
    bool startPositionSuccessfulWrite = writeNBitsShiftAndConvertToBigEndian64_new(
            outputFile, 40, region->start_pos
    );
    bool endPositionSuccessfulWrite = writeNBitsShiftAndConvertToBigEndian64_new(
            outputFile, 40, region->end_pos
    );
...
Region * parseRegion(InputBitstream *inputFile) {
    ...
    bool sequenceIdRead = readNBitsBigToNativeEndian16(
            inputFile,
            16,
            &sequence_Id
    );
    bool numberClassesRead = readNBits8(
            inputFile,
            4,
            &numberClasses
    );
    ...
    uint64_t start_pos;
    uint64_t end_pos;
    bool startPosRead = readNBitsBigToNativeEndian64(
            inputFile,
            40,
            &start_pos
    );
    bool endPosRead = readNBitsBigToNativeEndian64(
            inputFile,
            40,
            &end_pos
    );

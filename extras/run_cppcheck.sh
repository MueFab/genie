...
if [ ! -f $cppcheck ]; then
    printf "$SCRIPT_NAME: error: '$cppcheck' is not a regular file\n"
    exit -1
fi
...
for file in "${files[@]}"; do
    # printf "$SCRIPT_NAME: running cppcheck on: $file\n"
    ...
    $cppcheck $cppcheck_args -I $source_dir $file

    if [ $? -ne 0 ]; then
        printf "$SCRIPT_NAME: cppcheck returned with non-zero status\n"

/**
 * @file
 * @brief Gabac C wrapper. Compatible with ANSI C89.
...
/**
 * @brief Allocates a new data block and copies the contents from a source memory location
...
/**
 * @brief Resizes a data block. If the new size is bigger, the new space is not initiaized.
 * @param block
 * @param size New size
 * @return gabac_return_SUCCESS if no error occurred, gabac_return_FAILURE otherwise
 */
...
typedef enum gabac_transform
{
    ...
    gabac_transform_DIFF = 5,  /**< @brief Use differences between symbol values instead of symbols */
...
{
    void *data;  /**< @brief data_block* if input_mode == buffer and FILE* if input_mode == file */
...
/**
 * @brief Swap the block contents in a stream into an other data block. Does not work in file mode.
 * @param stream Stream
...
/**
 * @brief Swap the file pointer in a stream with an other file pointer. Does not work in block mode.
 * @param stream Stream

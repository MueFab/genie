    ...
    struct AccessUnitHeader {
        ...
        ParameterSet getParameterSet(std::vector<ParameterSet> collection) {
            for (size_t i = 0; i < collection.size(); ++i) {
                if (collection[i].parameter_set_ID == parameter_set_ID) return collection[i];
            }
    ...
    struct Block {
        ...
        struct BlockPayload {
            ...
            struct EncodedDescriptorSequences {
                    ...
                    EncodedDescriptorSequences(gabac::BitInputStream &instream, uint64_t AU_type, uint64_t
            descriptor_ID, uint64_t block_payload_size, uint64_t &bits_read_count,
            expandable_array<expandable_array<expandable_array<uint64_t> > > &decoded_symbols) { std::cout << "|
            AU_type, descriptor_ID: " << AU_type << ", " << descriptor_ID << std::endl;
                            ...
                            while(line_counter--) {
                                    ...
                                    std::cout << "| descID, subseqID, num_symbols: " << descID << ", " << subseqID << ",
            " << num_symbols << " > ";
            ...
            */
            ...
            BlockPayload() {}
            BlockPayload(gabac::BitInputStream &instream, uint64_t AU_type, uint64_t descriptor_ID,
                         uint64_t block_payload_size,
                         expandable_array<expandable_array<expandable_array<uint64_t> > > &decoded_symbols) {
                if (descriptor_ID == 11 || descriptor_ID == 15) {
                    ...
                    */
                    ...
                    encoded_descriptor_sequences = EncodedDescriptorSequences(instream, AU_type, descriptor_ID,
                    block_payload_size, bits_read_count, decoded_symbols);
                    */
        ...
        Block() {}
        Block(gabac::BitInputStream &instream, uint64_t AU_type,
              expandable_array<expandable_array<expandable_array<uint64_t> > > &decoded_symbols) {
            ...
            block_payload = BlockPayload(instream, AU_type, block_header.descriptor_ID, block_header.block_payload_size,
                                         decoded_symbols);
        }
    } * block;

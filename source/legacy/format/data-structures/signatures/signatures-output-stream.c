...
#include <stdlib.h>
#include <utils.h>
#include "signatures-output-stream.h"
...
SignaturesOutputStream* initSignaturesOutputStream(
    ...
    SignaturesOutputStream* signatureOutputStream =
            (SignaturesOutputStream*)malloc(sizeof(SignaturesOutputStream));
...
int writeSignatures(SignaturesOutputStream* signaturesOutputStream, Signatures* signatures){
    if(signatures->number_signatures != signaturesOutputStream->multipleSignatureBase){
        if(signatures->number_signatures > 0xffff){
            fprintf(stderr, "If the number of signatures is different to multiple signature base, then there cannot be "
                            "more then 0xffff signatures");
            return -1;
        }
        uint64_t valueToWrite = 0xffffffffffffffff;
        writeNBitsFromByteArray(
                signaturesOutputStream->outputBitstream,
                signaturesOutputStream->signatureSize,
                8,
    ...
    for(uint32_t signature_i=0; signature_i<signatures->number_signatures; signature_i++){
        ...
        if(signaturesOutputStream->u_signature_length != 0){
            if(signaturesOutputStream->u_signature_length != signatureToWrite->signatureLength){
                fprintf(
                        stderr, "The number of signature of symbols is set as constant, but the signature has a "
                                "different length");
                return -1;
            }
    ...
    for(uint32_t symbol_i=0; symbol_i<signatureToWrite->signatureLength; symbol_i++){
        if(symbolSize > bitsLeftInInteger){
            ...
            writeNBitsFromByteArray(
                    signaturesOutputStream->outputBitstream,
                    signaturesOutputStream->signatureSize,
                    sizeof(currentInteger),
                    (const uint8_t*)&currentInteger
            );

...
uint16_t getRefSequenceId(DatasetMasterIndexTable* datasetMasterIndexTable, uint16_t sequenceId, uint16_t classId,
                          uint32_t AU_id) {
    return datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequenceId]
        .classIndex[classId]
        .au_indexings[AU_id]
        .ref_sequence_id;
}
uint64_t getRefStartPosition(DatasetMasterIndexTable* datasetMasterIndexTable, uint16_t sequenceId, uint16_t classId,
                             uint32_t AU_id) {
    return datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequenceId]
        .classIndex[classId]
        .au_indexings[AU_id]
...
uint64_t getRefEndPosition(DatasetMasterIndexTable* datasetMasterIndexTable, uint16_t sequenceId, uint16_t classId,
                           uint32_t AU_id) {
    return datasetMasterIndexTable->sequence_indexes.sequence_indexes[sequenceId]
        .classIndex[classId]
        .au_indexings[AU_id]
...
void insertFinalOffset(DatasetMasterIndexTable* datasetMasterIndexTable, uint16_t classId, uint16_t descriptorId,
                       uint64_t offset) {
    TreeNode* treeNode = initTreeNode(offset);
    if (insert(datasetMasterIndexTable->offsetsPerClass->offsetsPerClass[classId].offsetsPerStream[descriptorId],
               treeNode)) {

...
DataUnits* initDataUnits() {
    ...
    dataUnits->allocatedDataUnitParametersIdsInfo.datasetgroupid =
        malloc(sizeof(uint8_t) * INITIAL_PARAMETER_IDS_INFO_ALLOCATION);
    dataUnits->allocatedDataUnitParametersIdsInfo.datasetid =
        malloc(sizeof(uint16_t) * INITIAL_PARAMETER_IDS_INFO_ALLOCATION);
...
bool getParametersIdDataUnitsScope(DataUnits* dataUnits, uint8_t datasetgroupId, uint16_t datasetId,
                                   uint8_t parameterIdDatasetScope, uint8_t* parameterIdDataUnitsScope) {
    for (size_t parameterIds_i = 0; parameterIds_i < dataUnits->allocatedDataUnitParametersIdsInfo.createdElements;
         parameterIds_i++) {
        if (datasetgroupId == dataUnits->allocatedDataUnitParametersIdsInfo.datasetgroupid[parameterIds_i] &&
            ...
            *parameterIdDataUnitsScope =
                dataUnits->allocatedDataUnitParametersIdsInfo.parameterSetId_DataUnitsScope[parameterIds_i];
            return true;
        }
    }
    return false;
...
uint8_t createParameterSetIdDataUnitsScope(DataUnits* dataUnits, uint8_t datasetgroupId, uint16_t datasetId,
    ...
    if (index == dataUnits->allocatedDataUnitParametersIdsInfo.allocatedElements) {
        dataUnits->allocatedDataUnitParametersIdsInfo.datasetgroupid =
            realloc(dataUnits->allocatedDataUnitParametersIdsInfo.datasetgroupid,
                    sizeof(uint8_t) * (index + INITIAL_PARAMETER_IDS_INFO_ALLOCATION));
        dataUnits->allocatedDataUnitParametersIdsInfo.datasetid =
            realloc(dataUnits->allocatedDataUnitParametersIdsInfo.datasetid,
                    sizeof(uint16_t) * (index + INITIAL_PARAMETER_IDS_INFO_ALLOCATION));
        dataUnits->allocatedDataUnitParametersIdsInfo.parameterSetId_DatasetScope =
            realloc(dataUnits->allocatedDataUnitParametersIdsInfo.parameterSetId_DatasetScope,
                    sizeof(uint16_t) * (index + INITIAL_PARAMETER_IDS_INFO_ALLOCATION));
        dataUnits->allocatedDataUnitParametersIdsInfo.parameterSetId_DataUnitsScope =
            realloc(dataUnits->allocatedDataUnitParametersIdsInfo.parameterSetId_DataUnitsScope,
                    sizeof(uint16_t) * (index + INITIAL_PARAMETER_IDS_INFO_ALLOCATION));
    }
...
int addDataUnitDatasetParameters(DataUnits* dataUnits, DatasetParameters* datasetParameters,
    ...
    if (!getParametersIdDataUnitsScope(dataUnits, datasetParameters->dataset_group_ID, datasetParameters->dataset_ID,
                                       datasetParameters->parent_parameter_set_ID, &parentParameterIdScopeDataUnits)) {
        return -1;
    }

    uint8_t parameterIdScopeDataUnits =
        createParameterSetIdDataUnitsScope(dataUnits, datasetParameters->dataset_group_ID,
                                           datasetParameters->dataset_ID, datasetParameters->parameter_set_ID);
    *parametersIdDataUnitsScope = parameterIdScopeDataUnits;

    DataUnitParametersSet* dataUnitParameterSet = initParametersSet(
        parentParameterIdScopeDataUnits, parameterIdScopeDataUnits, datasetParameters->encoding_parameters);

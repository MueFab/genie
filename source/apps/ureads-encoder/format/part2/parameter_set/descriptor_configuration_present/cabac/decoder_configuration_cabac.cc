    ...
    namespace desc_conf_pres {
        namespace cabac {
            DecoderConfigurationCabac::DecoderConfigurationCabac(GenomicDescriptor _desc)
                    : DecoderConfiguration(EncodingModeId::CABAC),
                      desc(_desc),
                ...
                for (size_t i = 0; i < getDescriptorProperties()[uint8_t(desc)].number_subsequences; ++i) {
                    descriptor_subsequence_cfgs.push_back(
                            make_unique<DescriptorSubsequenceCfg>(
                                    make_unique<TransformSubseqParameters>(),
                                    i,
            ...
            void DecoderConfigurationCabac::setSubsequenceCfg(uint8_t index,
                                                              std::unique_ptr<TransformSubseqParameters> cfg) {
                descriptor_subsequence_cfgs[index] =
                        make_unique<DescriptorSubsequenceCfg>(
                                std::move(cfg),
                                index,
                                desc == GenomicDescriptor::msar || desc == GenomicDescriptor::rname

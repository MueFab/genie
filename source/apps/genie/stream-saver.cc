...
void StreamSaver::compress(const std::string &name, gabac::DataBlock *data) { run_gabac(name, data, false); }
...
StreamSaver::StreamSaver(const std::string &configp, std::ostream *f, std::istream *ifi)
    : fout(f), fin(ifi), configPath(configp) {
    if (fin) {
        buildIndex();
    }
    reloadConfigSet();

...
// NOTE: right now I am using signed 64-bit integers for ALL variables; check section 13 at some point to use
// appropriate integer width for each variable
...
void decode(/* TODO: properly add parameters here */ /* access unit */ /* parameter set */) {
    ...
    if (numberOfTemplateSegments == 1)
        ...
        numberOfRecordSegments = 2;  // For now not going to be the case, since we are only concerned about Class P
    else if (subsequence0[j80] == 0)
    ...
    if (classId == Class_HM)
        numberOfAlignedRecordSegments = 1;  // Here for clarity; we are focusing on the Class P case for now
    else if (classId == Class_U)
        numberOfAlignedRecordSegments = 0;  // Same as above
    else
        numberOfAlignedRecordSegments = numberOfRecordSegments;  // Since we are in Class P for now (perfectly aligned
                                                                 // reads) this makes sense and will always be the case

    // Sub-Step 2.3: decoding unpairedRead
    int64_t unpairedRead;
    if (classId == Class_HM)
        unpairedRead = 0;  // Here for clarity
    else if (numberOfTemplateSegments == 1 || subsequence0[j80] == 5 || subsequence0[j80] == 6)
        unpairedRead = 1;
    else
        unpairedRead = 0;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // TODO: check if we should return them, or if they will be passed as parameters; this step might not be necessary
    // if the latter is the case
    std::vector<std::vector<std::string> > softClips(numberOfAlignedRecordSegments);
    std::vector<std::vector<int64_t> > softClipSizes(numberOfAlignedRecordSegments),
        hardClips(numberOfAlignedRecordSegments);  // Might be able to pull off using an integer type with less bits (to
                                                   // save space and stuff)
        ...
        // also what the frick is currentRecordCount
        {
            ...
            do {
                if (subsequence1[j51] <= 3) {
                    ...
                    do {
                        softClips[segmentIdx][leftRightIdx].push_back(
                            S[alphabet_ID][subsequence2[j52]]);  // NOTE: alphabet_ID is a parameter
                        j52++;
    ...
    if (read_length == 0) {
        for (int64_t i = 0; i < numberOfRecordSegments; ++i) readLength.push_back(subsequence0[j70++] + 1);
    } else {
    ...
    if (spliced_reads_flag && (classId == Class_I || classId == Class_HM)) {
        for (int64_t i = 0; i < numberOfAlignedRecordSegments; ++i) {
            ...
            do {
                ...
                // while(splicedSegLength[i].size() <= j) // ASK if this is intended; these arrays are too sparce if I
                // understand correctly. 	splicedSegLength.push_back(0);
    ...
    int64_t moreAlignments = 0, moreAlignmentsNextSeqId = 0, moreAlignmentsNextPos = 0;

    if (unpairedRead || classId == Class_HM) {
        numberOfAlignments = numberOfSegmentAlignments[0];
        for (int64_t i = 0; i < numberOfAlignments; ++i) alignPtr.push_back(std::vector<int64_t>(1, i));
    } else if (classId == class_U) {
        if (numberOfRecordSegments > 1) numberOfSegmentAlignments.push_back(0);
    ...
    if (multiple_alignment_flag == 1 && classId != Class_U && subsequence2[j102++])
    ...
    */
    ...
    if (classId != Class_HM) {
        for (int64_t i = 1; i < numberOfTemplateSegments; ++i) {
            if (subsequence0[j80] == 0) {
                ...
                if (classId !=
                    Class_U) {  // Note: in specification this is referred to as CLASS_U for some reason. Typo?
    ...
    for (int64_t i = 1; i < numberOfSegmentAlignments[0]; ++i) splitMate.push_back(std::vector<int64_t>(1, 0));

    if ((classId == class_P || classId == class_N || classId == Class_M || classId == class_I) && !unpairedRead) {
        for (int64_t j = 1; j < numberOfTemplateSegments; ++j) {
            ...
            for (int64_t i = 1; i < numberOfAlignments; ++i) {
                ...
                if (alignIdx > currAlignIdx) {
                    ...
                    if (subsequence0[j80] == 0) {
                        splitMate[alignIdx][j] =
                            0;  // TODO: THIS MIGHT CAUSE A RUNTIME ERROR; to fix re-write code using expandable_array
                        delta = subsequence1[j81] >> 1;

                        if (subsequence1[j81] & 0x0001) delta = -delta;
    ...
    for (int64_t i = 0; i < numberOfAlignedRecordSegments; ++i) splicedSegMappingPos[i][0] = mappingPos[0][i];
    ...
    */
    ...
    std::vector<std::vector<std::vector<int64_t> > >
        reverseComp;  // NOTE: uninitialized; change to expandable_array and everything will work nicely (:

    for (int64_t i = 0; i < numberOfAlignedRecordSegments; ++i) {
        for (int64_t j = 0; j < numberOfSegmentAlignments[i]; ++j) {
            if (splitMate[j][i] == 0) {
                for (int64_t k = 0; k < numberOfSplicedSeg[i]; ++k) reverseComp[k][j][i] = subsequence0[j10++];
            }
        }
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int64_t i = 0; i < as_depth; ++i) {
        for (int64_t j = 0; j < numberOfAlignedRecordSegments; ++j) {
            for (int64_t k = 0; k < numberOfSegmentAlignments[j]; ++k) {
                if (splitMate[k][j] == 0) mappingScores[k][j][i] = subsequence0[j90]++;
            }
        }
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (multiple_alignments_flag) {
        ...
        for (int64_t i = 0; i < numberOfAlignedRecordSegments; ++i) {
            for (int64_t j = 0; j < numberOfSegmentAlignments[i] - 1; ++j) {
                if (splitMate[j][i] == 0) decodedMsar[j][i] = decodedStrings[k++];
            }
        }
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int64_t tSeg = 0; tSeg < numberOfTemplateSegments; ++tSeg) {
        for (int64_t qs = 0; qs < qv_depth; ++qs) {
            if (j140 < subsequence0.size()) {  // ask if this is a vector; if not, convert to vector (or even better use
                                               // expandable_array)
            ...
            if (qvPresentFlag == 1) {
                ...
                for (int64_t baseIdx = 0; baseIdx < numBases; ++baseIdx) {
                    if ((classId == Class_I || classId == Class_HM) &&
                        !isAligned(baseIdx)) {  // is this already implemented for me ?? if not how do i do that??

...
std::string extract_tok_value(data_structures::expandable_array<data_structures::expandable_array<uint64_t> >, uint64_t,
                              uint64_t, uint64_t);
...
struct block_header {
    uint8_t descriptor_ID : uint32_t block_payload_size;
};
...
struct encoding_parameters {
    ...
    uint8_t number_of_template_segments_minus1 uint32_t max_au_data_unit_size;

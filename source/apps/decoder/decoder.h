...
void Decode(AccessUnit &AU, std::vector<ParameterSet> collection, RawReference RR, uint64_t &previousMappingPos0) {
    ...
    uint64_t classId                = AU.access_unit_header.AU_type; // 1
    ...
    uint64_t numberOfTemplateSegments    = PS.encoding_parameters.number_of_template_segments_minus1+1;
    ...
    expandable_array<expandable_array<expandable_array<uint64_t> > > decoded_symbols = AU.decoded_symbols;
    ...
    while(reads_count > 0) {
	    ...
	    std::cout << "number_of_template_segments: " << numberOfTemplateSegments << std::endl;
		std::cout << "number_of_record_segments: " << numberOfRecordSegments << std::endl;

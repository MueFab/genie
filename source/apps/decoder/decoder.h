...
void getDecodedSymbols(AccessUnit &AU, std::vector<ParameterSet> collection, RawReference RR, uint64_t &previousMappingPos0,
						expandable_array<expandable_array<expandable_array<uint64_t> > > &decoded_symbols) {
	decoded_symbols = AU.decoded_symbols;
}

void decodeNumberOfSegmentVariables(AccessUnit &AU, std::vector<ParameterSet> collection, RawReference RR, uint64_t &previousMappingPos0, 
							expandable_array<expandable_array<expandable_array<uint64_t> > > decoded_symbols,
...
void decodeMMAP(AccessUnit &AU, std::vector<ParameterSet> collection, RawReference RR, uint64_t &previousMappingPos0, 
    ...
    if(unpairedRead || classId == CLASS_HM) {
        ...
        for(size_t i=0; i<numberOfAlignments; ++i)
            alignPtr[i][0] = i;
    }
...
void decodeSplicedSequence(AccessUnit &AU, std::vector<ParameterSet> collection, RawReference RR, uint64_t &previousMappingPos0, 
					expandable_array<expandable_array<expandable_array<uint64_t> > > decoded_symbols,
					uint64_t numberOfMappedRecordSegments,
					expandable_array<uint64_t>   numberOfSplicedSeg,
					expandable_array<expandable_array<uint64_t> > mappingPos,
    ...
    for(size_t i=0; i<numberOfMappedRecordSegments; ++i) {
    	for(size_t j=0; j<numberOfSplicedSeg[i]; ++j) {
    		...
    		if(classId == CLASS_I || classId == CLASS_HM) {
    			if(j == 0)
    				mappedLength -= softClipSizes[i][0];
	    		if(j == numberOfSplicedSeg[i] - 1) 
...
void decode(AccessUnit &AU, std::vector<ParameterSet> collection, RawReference RR, uint64_t &previousMappingPos0) {
    ...
    while(reads_count > 0) {
    	...
    	decodeNumberOfSegmentVariables(AU, collection, RR, previousMappingPos0, decoded_symbols, numberOfTemplateSegments, numberOfRecordSegments, numberOfAlignedRecordSegments, unpairedRead);
		decodeCLIPS(AU, collection, RR, previousMappingPos0, decoded_symbols, numberOfAlignedRecordSegments, softClips, softClipSizes, hardClips, S, currentRecordCount);
		decodeRLEN(AU, collection, RR, previousMappingPos0, decoded_symbols, numberOfRecordSegments, numberOfAlignedRecordSegments, hardClips, readLength, numberOfSplicedSeg, splicedSegLength);
	   	decodeMMAP(AU, collection, RR, previousMappingPos0, decoded_symbols, numberOfRecordSegments, unpairedRead, numberOfAlignments, numberOfSegmentAlignments, numberOfAlignmentPairs, alignPtr, moreAlignments, moreAlignmentsNextPos, moreAlignmentsNextSeqId);
	   	decodePOS(AU, collection, RR, previousMappingPos0, decoded_symbols, numberOfSegmentAlignments, mappingPos);
	    decodePAIR(AU, collection, RR, previousMappingPos0, decoded_symbols, numberOfTemplateSegments, mappingPos, read1First, delta, splitMate, splicedSegMappingPos, mateSeqId, mateRecordIndex, mateAuId);
	    decodeRCOMP(AU, collection, RR, previousMappingPos0, decoded_symbols, mappingPos, numberOfRecordSegments, numberOfAlignedRecordSegments, numberOfSegmentAlignments, splitMate, numberOfSplicedSeg, reverseComp, numberOfSegmentMappings, numberOfMappedRecordSegments);
	    decodeRGROUP(AU, collection, RR, previousMappingPos0, decoded_symbols, readGroupId, has_readGroupId);
	    decodeSplicedSequence(AU, collection, RR, previousMappingPos0, decoded_symbols, numberOfMappedRecordSegments, numberOfSplicedSeg, mappingPos, softClipSizes, splicedSegLength, splicedSequence, pRef, mappedLength);

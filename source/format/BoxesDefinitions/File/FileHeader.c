...
void freeFileHeader(FileHeader* fileHeader){
    for(int i=0; i<getSize(fileHeader->compatibleBrands); i++ ){
...
bool writeFileHeader(FileHeader* fileHeader, FILE* outputFile){
    ...
    if(
        !boxNameSuccessfulWrite ||
        !fileHeaderSuccessful ||
        !writeFileHeaderContent(fileHeader, outputFile)
    ){
...
uint64_t getSizeFileHeader(FileHeader* fileHeader){
    ...
    return sizeMajorBrand
           + sizeMinorVersion
           + sizeCompatibleBrands;
}

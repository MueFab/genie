...
bitset mask63;  // bitset with 64 bits set to 1 (used in bitsettostring for
                // conversion to ullong)

void decode();

void restore_order(std::string outfile, std::string infile_order, int filenum,
                   uint32_t *index_array, bitset *reads_bin,
                   uint8_t *read_lengths_bin);
// filenum 1 or 2

void modify_id(std::string &id, uint8_t paired_id_code);

void write_fastq_block(char *read, std::string &quality_file_prefix,
                       std::string &id_file_prefix, std::ifstream &fin_quality,
                       std::ifstream &fin_id, int &current_tid_quality,
                       int &current_tid_id, std::ofstream &fout,
                       bool &paired_id_match, int filenum, uint8_t cur_readlen);
...
int main(int argc, char **argv) {
  ...
  max_bin_size =
      12000000000 / (sizeof(bitset) + sizeof(uint32_t) + sizeof(uint8_t));
  if (max_bin_size > numreads_by_2 || fast_mode == "True")
    max_bin_size =
        numreads_by_2 +
        1;  //+1 just to make sure there is only one pass over the data
  ...
  restore_order(outfile_2, outfile_order_2, 2, index_array, reads_bin,
                read_lengths_bin);
  if (preserve_order == "True")
    restore_order(outfile_1, infile_order, 1, index_array, reads_bin,
                  read_lengths_bin);
  delete[] index_array;
  delete[] reads_bin;
...
// first calculate numreads in each thread, this is needed for accessing
// flag_first, readlength
#pragma omp parallel
  {
...
#pragma omp parallel
  {
    ...
    if (tid != 0) {
      for (int i = 0; i < tid; i++) pos_in_reordered_reads += numreads_thr[i];
    }
    ...
    for (int tid_e = tid * num_thr_e / num_thr;
      ...
      std::ofstream f_2(outfile_2 + '.' + std::to_string(tid_e),
                        std::ios::binary);
      ...
      char currentread[MAX_READ_LEN + 1], ref[MAX_READ_LEN + 1],
          revread[MAX_READ_LEN + 1];
      bitset b;
      std::string noise;
      char c;
      long pos;
      ...
      while (f_pos >> std::noskipws >> c) {  // don't skip whitespaces
        ...
        if (pos == max_readlen) {
          ...
          if (pos != 0)
            for (int i = 0; i <= ref_len - 1 - pos; i++) ref[i] = ref[i + pos];
  ...
  if (preserve_order == "True") {
    ...
    for (int tid_e = 0; tid_e < num_thr_e; tid_e++) {
      std::ifstream f_in_1(outfile_1 + '.' + std::to_string(tid_e),
                           std::ios::binary);
      std::ifstream f_in_2(outfile_2 + '.' + std::to_string(tid_e),
                           std::ios::binary);
      f_1 << f_in_1.rdbuf();
      f_2 << f_in_2.rdbuf();
    ...
    for (int i = 0; i < num_thr; i++) pos_in_reorder_reads += numreads_thr[i];
    std::ifstream f_readlength(infile_readlength, std::ios::binary);
    std::ifstream f_flag_first(infile_paired_flag_first);
    ...
    if (preserve_quality == "True")
      fin_quality.open(quality_file_prefix + "." +
                       std::to_string(current_tid_quality));
    if (preserve_id == "True")
    ...
    for (int tid_e = 0; tid_e < num_thr_e; tid_e++) {
      ...
      while (!f_in_1.eof()) {
        ...
        write_fastq_block(currentread, quality_file_prefix, id_file_prefix,
                          fin_quality, fin_id, current_tid_quality,
                          current_tid_id, f_1, paired_id_match, 1, cur_readlen);
        f_in_1.read((char *)&cur_readlen, sizeof(uint8_t));
      }
    ...
    if (!f_readlength.eof()) {
      ...
      while (!f_singleton.eof()) {
        ...
        if (flag_first == '1') {
          write_fastq_block(currentread, quality_file_prefix, id_file_prefix,
                            fin_quality, fin_id, current_tid_quality,
                            current_tid_id, f_1, paired_id_match, 1,
                            cur_readlen);
        } else {
          b = chartobitset(currentread, cur_readlen);
          f_2.write((char *)&cur_readlen, sizeof(uint8_t));
    ...
    f_flag_first.close();
    if (preserve_quality == "True")
      remove((quality_file_prefix + "." + std::to_string(current_tid_quality))
                 .c_str());
  }
...
void restore_order(std::string outfile, std::string infile_order, int filenum,
  ...
  for (uint32_t i = 0; i <= numreads_by_2 / max_bin_size; i++) {
    ...
    for (uint32_t j = 0; j < numreads_by_2; j++) {
      ...
      if (order >= i * max_bin_size &&
          order < i * max_bin_size + numreads_bin) {
    ...
    for (uint32_t j = 0; j < numreads_bin; j++) {
      bitsettostring(reads_bin[index_array[j]], s,
                     read_lengths_bin[index_array[j]]);
      write_fastq_block(s, quality_file_prefix, id_file_prefix, fin_quality,
                        fin_id, current_tid_quality, current_tid_id, fout,
                        paired_id_match, filenum,
                        read_lengths_bin[index_array[j]]);
    }
...
void write_fastq_block(char *read, std::string &quality_file_prefix,
                       ...
                       std::ifstream &fin_id, int &current_tid_quality,
                       int &current_tid_id, std::ofstream &fout,
                       bool &paired_id_match, int filenum,
                       uint8_t cur_readlen) {
  std::string id;
  ...
  if (preserve_id == "True") {
    ...
    if (paired_id_match) modify_id(id, paired_id_code);
  }
...
#pragma omp parallel
  {
    ...
    for (int tid_e = tid * num_thr_e / num_thr;
      ...
      std::ifstream in_seq_tail(infile_seq + '.' + std::to_string(tid_e) +
                                ".tail");
      std::ifstream in_rev(infile_rev + '.' + std::to_string(tid_e),
                           std::ios::binary);
      std::ifstream in_rev_tail(infile_rev + '.' + std::to_string(tid_e) +
                                ".tail");
      char inttobase[4];
      inttobase[0] = 'A';
      inttobase[1] = 'C';
      ...
      rename((infile_rev + '.' + std::to_string(tid_e) + ".tmp").c_str(),
             (infile_rev + '.' + std::to_string(tid_e)).c_str());
    }  // for end
  }    // parallel end

...
struct reorder_global {
	...
	std::bitset<bitset_size> basemask[MAX_READ_LEN][128];  // bitset for A,G,C,T at each position
	// used in stringtobitset, chartobitset and bitsettostring
	std::bitset<bitset_size> mask64;  // bitset with 64 bits set to 1 (used in bitsettostring for
			// conversion to ullong)
};


template<size_t bitset_size>
void bitsettostring(std::bitset<bitset_size> b, char *s, uint8_t readlen, reorder_global<bitset_size> &rg) {
...
void updaterefcount(std::bitset<bitset_size> &cur, std::bitset<bitset_size> &ref, std::bitset<bitset_size> &revref,
                    ...
                    int shift, uint8_t cur_readlen, int &ref_len, reorder_global<bitset_size> &rg)
    ...
    } else  // reverse case is quite complicated in the variable length case
    {
      if (cur_readlen - shift >= ref_len) {
        ...
        for (int i = ref_len - cur_readlen + shift; i < ref_len; i++)
          count[rg.chartoint[(uint8_t)current[i - (ref_len - cur_readlen + shift)]]][i] +=
              1;
        for (int i = ref_len; i < ref_len + shift; i++) {
          for (int j = 0; j < 4; j++) count[j][i] = 0;
          count[rg.chartoint[(uint8_t)current[i - (ref_len - cur_readlen + shift)]]][i] = 1;
        }
        ...
        for (int i = rg.max_readlen - cur_readlen; i < rg.max_readlen - shift; i++)
          count[rg.chartoint[(uint8_t)current[i - (rg.max_readlen - cur_readlen)]]][i] += 1;
...
template<size_t bitset_size>
void readDnaFile(std::bitset<bitset_size> *read, uint8_t *read_lengths, reorder_global<bitset_size> &rg) {
#pragma omp parallel
  {
...
template<size_t bitset_size>
bool search_match(std::bitset<bitset_size> &ref, std::bitset<bitset_size> *mask1, omp_lock_t *dict_lock,
                  omp_lock_t *read_lock,
                  std::bitset<bitset_size> mask[MAX_READ_LEN][MAX_READ_LEN],
                  uint8_t *read_lengths, bool *remainingreads, std::bitset<bitset_size> *read,
...
template<size_t bitset_size>
void reorder(std::bitset<bitset_size> *read, bbhashdict *dict, uint8_t *read_lengths, reorder_global<bitset_size> &rg) {
...
#pragma omp barrier
    ...
    while (!done) {
      ...
      for (int shift = 0; shift < rg.maxshift; shift++) {
        // find forward match
        flag =
            search_match<bitset_size>(ref, mask1, dict_lock, read_lock, mask, read_lengths,
        ...
        // find reverse match
        flag = search_match<bitset_size>(revref, mask1, dict_lock, read_lock, mask,
...
template<size_t bitset_size>
void writetofile(std::bitset<bitset_size> *read, uint8_t *read_lengths, reorder_global<bitset_size> &rg) {

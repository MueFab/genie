...
int main(int argc, char **argv) {
  ...
  if (preserve_quality == "True") quality_mode = std::string(argv[8]);
  ...
  if (preserve_quality == "True" &&
      !(quality_mode == "bsc" || quality_mode == "illumina_binning_bsc")) {
...
#pragma omp parallel
    {
      ...
      for (int tid_e = tid * num_thr_e / num_thr;
        ...
        comp_info.fcomp =
            fopen((infile_id[k] + "." + std::to_string(tid_e)).c_str(), "r");
        comp_info.f_id =
            fopen((outfile_id[k] + "." + std::to_string(tid_e)).c_str(), "w");
...
#pragma omp parallel
    {
      ...
      for (int tid_e = tid * num_thr_e / num_thr;
        ...
        std::string input_file_string =
            (infile_quality[k] + "." + std::to_string(tid_e));
        ...
        std::string output_file_string =
            (outfile_quality[k] + "." + std::to_string(tid_e));

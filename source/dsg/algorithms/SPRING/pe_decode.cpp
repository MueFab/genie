...
void generate_order_from_paired() {
  ...
  rename((infile_paired_flag_first + ".tmp").c_str(),
         infile_paired_flag_first.c_str());

  // now we'll need the inverse index, before doing that write to file to save
  // memory
  ...
  // now, go through the reads in order, and for each read which is 1st in
  // paired end,
  ...
  if (preserve_order == "True")  // in this case, we need to map the order
                                 // obtained above to the true positions
  {
    ...
    f_temp.open(outfile_order_2, std::ios::binary);
    for (uint32_t i = 0; i < numreads_by_2; i++)
      f_temp.write((char *)&read_order[numreads_by_2 + read_order[i]],
                   sizeof(uint32_t));
    f_temp.close();
  }
  delete[] read_order;

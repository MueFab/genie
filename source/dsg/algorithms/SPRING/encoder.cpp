...
std::string buildcontig(std::list<contig_reads> &current_contig,
                        uint32_t list_size);  // using lists to enable faster
...
#pragma omp parallel
  {
    ...
    std::ifstream in_pos(infile_pos + '.' + std::to_string(tid),
                         std::ios::binary);
    std::ifstream in_order(infile_order + '.' + std::to_string(tid),
                           std::ios::binary);
    std::ifstream in_RC(infile_RC + '.' + std::to_string(tid));
    std::ifstream in_readlength(infile_readlength + '.' + std::to_string(tid),
                                std::ios::binary);
    ...
    std::ofstream f_order(infile_order + '.' + std::to_string(tid) + ".tmp",
                          std::ios::binary);
    std::ofstream f_RC(infile_RC + '.' + std::to_string(tid) + ".tmp");
    std::ofstream f_readlength(
        infile_readlength + '.' + std::to_string(tid) + ".tmp",
        std::ios::binary);
                                                     ...
                                                     // too large
      {
        if (list_size != 0) {
          ...
          if (ref.size() >= max_readlen) {
            stringtobitset(ref.substr(0, max_readlen), max_readlen,
                           forward_bitset);
            stringtobitset(
                reverse_complement(ref.substr(0, max_readlen), max_readlen),
                max_readlen, reverse_bitset);
            for (long j = 0; j < ref.size() - max_readlen + 1; j++) {
                    ...
                    for (int64_t i = dictidx[1] - 1;
                      ...
                      if (!rev)
                        hamming = ((forward_bitset ^ read[rid]) &
                                   mask[0][max_readlen - read_lengths_s[rid]])
                                      .count();
                      else
                      ...
                      if (flag == 1)  // match found
                      {
                        ...
                        char rc = rev ? 'r' : 'd';
                        long pos =
                            rev ? (j + max_readlen - read_lengths_s[rid]) : j;
              ...
              if (j !=
                  ref.size() -
                      max_readlen)  // not at last position,shift bitsets
              {
                forward_bitset >>= 3;
                forward_bitset = forward_bitset & mask[0][0];
                forward_bitset |=
                    basemask[max_readlen - 1][ref[j + max_readlen]];
                reverse_bitset <<= 3;
                reverse_bitset = reverse_bitset & mask[0][0];
                reverse_bitset |=
                    basemask[0][chartorevchar[ref[j + max_readlen]]];
              }
  ...
  for (int tid = 0; tid < num_thr; tid++) {
    ...
    rename((infile_RC + '.' + std::to_string(tid) + ".tmp").c_str(),
           (infile_RC + '.' + std::to_string(tid)).c_str());
...
void generatemasks(bitset mask[MAX_READ_LEN][MAX_READ_LEN]) {
  for (int i = 0; i < max_readlen; i++) {
    for (int j = 0; j < max_readlen; j++) {
      mask[i][j].reset();
      for (int k = 3 * i; k < 3 * max_readlen - 3 * j; k++) mask[i][j][k] = 1;
    }
  }
  return;
}
...
#pragma omp parallel
  {
    ...
    std::ofstream f_seq(outfile_seq + '.' + std::to_string(tid) + ".tmp",
                        std::ios::binary);
    ...
    rename((outfile_seq + '.' + std::to_string(tid) + ".tmp").c_str(),
           (outfile_seq + '.' + std::to_string(tid)).c_str());

    // rev
    std::ifstream in_rev(infile_RC + '.' + std::to_string(tid));
    std::ofstream f_rev(infile_RC + '.' + std::to_string(tid) + ".tmp",
                        std::ios::binary);
...
void getDataParams() {
  ...
  numreads_s = 0;
  std::string line;
  while (std::getline(myfile_s, line)) ++numreads_s;
  myfile_s.close();
...
void correct_order(uint32_t *order_s) {
  ...
  for (int tid = 0; tid < num_thr; tid++) {
    std::ifstream fin_order(infile_order + '.' + std::to_string(tid),
                            std::ios::binary);
    std::ofstream fout_order(infile_order + '.' + std::to_string(tid) + ".tmp",
                             std::ios::binary);
    uint32_t pos;
    ...
    fout_order.close();
    remove((infile_order + '.' + std::to_string(tid)).c_str());
    rename((infile_order + '.' + std::to_string(tid) + ".tmp").c_str(),
           (infile_order + '.' + std::to_string(tid)).c_str());
  }
...
{
  for (int j = 0; j < numdict_s; j++) mask1[j].reset();
...
#pragma omp parallel
    {
      ...
      stop =
          uint64_t(tid + 1) * (numreads_s + numreads_N) / omp_get_num_threads();
      if (tid == omp_get_num_threads() - 1) stop = numreads_s + numreads_N;
...
#pragma omp for
    for (int j = 0; j < numdict_s; j++) {
      ...
      for (int tid = 0; tid < num_thr; tid++) {
        ...
        finhash.read((char *)&currenthash, sizeof(uint64_t));
        while (!finhash.eof()) {
          while (read_lengths_s[i] <= dict_end[j]) i++;

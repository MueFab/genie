...
void updaterefcount(bitset &cur, bitset &ref, bitset &revref,
                    ...
                    int shift, uint8_t cur_readlen, int &ref_len);
...
#pragma omp parallel
  {
    ...
    std::ofstream foutpos(outfilepos + '.' + tid_str,
                          std::ofstream::out | std::ios::binary);
    std::ofstream foutorder(outfileorder + '.' + tid_str,
                            std::ofstream::out | std::ios::binary);
    std::ofstream foutorder_s(outfileorder + ".singleton." + tid_str,
                              std::ofstream::out | std::ios::binary);
    std::ofstream foutlength(outfilereadlength + '.' + tid_str,
                             std::ofstream::out | std::ios::binary);
...
#pragma omp critical
    {  // doing initial setup and first read
      current = firstread;
      firstread +=
          numreads / omp_get_num_threads();  // spread out first read equally
      remainingreads[current] = 0;
      unmatched[tid]++;
    }
#pragma omp barrier
    updaterefcount(read[current], ref, revref, count, true, false, 0,
                   read_lengths[current], ref_len);
    cur_read_pos = 0;
    ref_pos = 0;
    first_rid = current;
    prev_unmatched = true;
    ...
    while (!done) {
      ...
      if (!left_search_start) {
        for (int l = 0; l < numdict; l++) {
          if (read_lengths[current] <= dict_end[l]) continue;
      ...
      for (int shift = 0; shift < maxshift; shift++) {
        ...
        flag =
            search_match(ref, mask1, dict_lock, read_lock, mask, read_lengths,
                         remainingreads, read, dict, k, false, shift, ref_len);
        if (flag == 1) {
          current = k;
      ...
      if (flag == 0)  // no match found
      {
        if (!left_search)  // start left search
        {
          ...
          updaterefcount(read[first_rid], ref, revref, count, true, true, 0,
                         read_lengths[first_rid], ref_len);
          ref_pos = 0;
          cur_read_pos = 0;
...
void generatemasks(bitset mask[MAX_READ_LEN][MAX_READ_LEN]) {
  for (int i = 0; i < max_readlen; i++) {
    for (int j = 0; j < max_readlen; j++) {
      mask[i][j].reset();
      for (int k = 2 * i; k < 2 * max_readlen - 2 * j; k++) mask[i][j][k] = 1;
    }
  }
  return;
}
...
bool search_match(bitset &ref, bitset *mask1, omp_lock_t *dict_lock,
  ...
  for (int l = 0; l < numdict; l++) {
    ...
    if (!rev) {
      if (dict_end[l] + shift >= ref_len) continue;
    } else {
      if (dict_end[l] >= ref_len + shift || dict_start[l] <= shift) continue;
    }
    b = ref & mask1[l];
    ull = (b >> 2 * dict_start[l]).to_ullong();
    startposidx = dict[l].bphf->lookup(ull);
    ...
    uint64_t ull1 =
        ((read[dict[l].read_id[dictidx[0]]] & mask1[l]) >> 2 * dict_start[l])
            .to_ullong();
...
#pragma omp parallel
  {
    ...
    std::ifstream finorder(outfileorder + '.' + tid_str,
                           std::ifstream::in | std::ios::binary);
    std::ifstream finorder_s(outfileorder + ".singleton." + tid_str,
                             std::ifstream::in | std::ios::binary);
  ...
  std::ofstream foutorder_s(outfileorder + ".singleton",
                            std::ofstream::out | std::ios::binary);
  for (int tid = 0; tid < num_thr; tid++) {
    std::string tid_str = std::to_string(tid);
  ...
  if (resetcount == true)  // resetcount - unmatched read so start over
  {
    ...
    if (!rev) {
      ...
      for (int i = 0; i < ref_len - shift; i++) {
        ...
        if (i < cur_readlen) count[chartoint[current[i]]][i] += 1;
      }
    ...
    } else  // reverse case is quite complicated in the variable length case
    {
      if (cur_readlen - shift >= ref_len) {
        ...
        for (int i = ref_len - cur_readlen + shift; i < ref_len; i++)
          count[chartoint[current[i - (ref_len - cur_readlen + shift)]]][i] +=
              1;

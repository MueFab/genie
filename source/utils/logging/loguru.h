...
namespace loguru
{
	...
	Verbosity g_stderr_verbosity  = Verbosity_EMERGENCY;
...
#define VTSEQ(ID) ("\e[" #ID "m")
	...
	void file_reopen(void* user_data)
	{
		...
		if (!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 || (st.st_ino != file_abs->st.st_ino)) {
			...
			if (!file_abs->fp) {
				LOG_F(INFORMATIONAL, "Reopening file '%s' due to previous error", file_abs->path);
			}
			else if (ret < 0) {
				const auto why = errno_as_text();
				LOG_F(INFORMATIONAL, "Reopening file '%s' due to '%s'", file_abs->path, why.c_str());
			} else {
				LOG_F(INFORMATIONAL, "Reopening file '%s' due to file changed", file_abs->path);
			}
		...
		for (int arg_it = 1; arg_it < argc; ++arg_it) {
			...
			if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !std::isalpha(cmd[arg_len], std::locale(""))) {
				...
				if (strcmp(value_str, "OFF") == 0) {
					...
					g_stderr_verbosity = Verbosity_INFORMATIONAL;
				...
				} else if (strcmp(value_str, "EMERGENCY") == 0) {
					g_stderr_verbosity = Verbosity_EMERGENCY;
				} else {
					char* end = 0;
	...
	void init(int& argc, char* argv[], const char* verbosity_flag)
	{
		...
		LOG_F(INFORMATIONAL, "arguments: %s", s_arguments.c_str());
		if (strlen(s_current_dir) != 0)
		{
			LOG_F(INFORMATIONAL, "Current dir: %s", s_current_dir);
		}
		LOG_F(INFORMATIONAL, "stderr verbosity: %d", g_stderr_verbosity);
		LOG_F(INFORMATIONAL, "-----------------------------------");
	...
	void shutdown()
	{
		LOG_F(INFORMATIONAL, "loguru::shutdown()");
	...
	bool add_file(const char* path_in, FileMode mode, Verbosity verbosity)
	{
		...
		LOG_F(INFORMATIONAL, "Logging to '%s', mode: '%s', verbosity: %d", path, mode_str, verbosity);
		return true;
	}
	...
	static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
	{
		...
		}  else if (verbosity == Verbosity_EMERGENCY) {
			snprintf(level_buff, sizeof(level_buff) - 1, "EMGY");
		} else if (verbosity == Verbosity_ALERT) {
			snprintf(level_buff, sizeof(level_buff) - 1, "ALERT");
		} else if (verbosity == Verbosity_CRITICAL) {
			snprintf(level_buff, sizeof(level_buff) - 1, "CRIT");
		} else if (verbosity == Verbosity_ERROR) {
			snprintf(level_buff, sizeof(level_buff) - 1, "ERROR");
		} else if (verbosity == Verbosity_WARNING) {
			snprintf(level_buff, sizeof(level_buff) - 1, "WARN");
		} else if (verbosity == Verbosity_NOTICE) {
			snprintf(level_buff, sizeof(level_buff) - 1, "NOTE");
		} else if (verbosity == Verbosity_INFORMATIONAL) {
	...
	static void log_message(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal)
	{
		...
		if (message.verbosity == Verbosity_EMERGENCY) {
	...
	void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, const char* format, ...)
	{
		...
		va_start(vlist, format);
		auto buff = vtextprintf(format, vlist);
		log_to_everywhere(stack_trace_skip + 1, Verbosity_EMERGENCY, file, line, expr, buff.c_str());
		va_end(vlist);
...
namespace loguru
{
	...
	void signal_handler(int signal_number, siginfo_t*, void*)
	{
		...
		*/
		...
		flush();
		char preamble_buff[LOGURU_PREAMBLE_WIDTH];
		print_preamble(preamble_buff, sizeof(preamble_buff), Verbosity_EMERGENCY, "", 0);
		auto message = Message{Verbosity_EMERGENCY, "", 0, preamble_buff, "", "Signal: ", signal_name};
		try {

...
const AnalysisConfiguration& getCandidateConfig() {
    static const AnalysisConfiguration config = {
        ...
        {// Binarizations (unsigned)
         gabac::BinarizationId::BI, gabac::BinarizationId::TU,
         gabac::BinarizationId::EG, gabac::BinarizationId::TEG},
        {// Binarizations (signed)
         gabac::BinarizationId::SEG, gabac::BinarizationId::STEG},
        {// Binarization parameters (TEG and STEG only)
         1, 2, 3, 5, 7, 9, 15, 30, 255},
        {// Context modes
...
void getOptimumOfBinarizationParameter(const AnalysisConfiguration& aconf,
    ...
    for (const auto& transID : aconf.candidateContextSelectionIds) {
        info->currConfig
            .transformedSequenceConfigurations[info->currStreamIndex]
            .contextSelectionId = transID;
        info->stack.push(info->stack.top());
        size_t maxSize =
            std::min(info->bestSequenceSize - info->currSequenceSize,
        ...
        info->currSequenceSize +=
            sizeof(uint32_t) + info->stack.top().streams.front().size();
...
static void getMinMax(const gabac::DataBlock& b, uint64_t* umin, uint64_t* umax,
...
static uint64_t getMaxOfStream(SequenceTransformationId trans, uint8_t stream,
                               uint64_t max) {
    if ((trans == SequenceTransformationId::rle_coding ||
         trans == SequenceTransformationId::match_coding) &&
        stream > 0)
...
void getOptimumOfBinarization(const AnalysisConfiguration& aconf,
    ...
    if (aconf.maxValue == 0) {
        ...
        max = getMaxOfStream(info->currConfig.sequenceTransformationId,
                             info->currStreamIndex, aconf.maxValue);
        min = 0;
        smax = max;
        smin = -smax;
    }

    const unsigned BIPARAM =
        (max > 0) ? unsigned(std::floor(std::log2(max)) + 1) : 1;
    const unsigned TUPARAM =
        (max > 0) ? unsigned(std::min(max, uint64_t(256))) : 1;
    const std::vector<std::vector<unsigned>> candidates = {
        {std::min(BIPARAM, 32u)},
        {std::min(TUPARAM, 32u)},
        {0},
        {0},
    ...
    for (const auto& transID : candidates[unsigned(id)]) {
        if (!getBinarization(id).isSigned) {
            ...
            if (!getBinarization(id).sbCheck(uint64_t(smin), uint64_t(smax),
                                             transID)) {
                continue;
            }
        }
        ...
        info->currConfig
            .transformedSequenceConfigurations[info->currStreamIndex]
            .binarizationParameters = {transID};

        getOptimumOfBinarizationParameter(aconf, info);
    }
}

void getOptimumOfDiffTransformedStream(const AnalysisConfiguration& aconf,
                                       TraversalInfo* info) {
    std::vector<gabac::BinarizationId> candidates =
        (!info->currConfig
              .transformedSequenceConfigurations[info->currStreamIndex]
              .diffCodingEnabled)
    ...
    for (const auto& transID : candidates) {
        info->currConfig
            .transformedSequenceConfigurations[info->currStreamIndex]
            .binarizationId = transID;
        getOptimumOfBinarization(aconf, info);
    }
}
...
void getOptimumOfLutTransformedStream(const AnalysisConfiguration& aconf,
    ...
    for (const auto& transID : aconf.candidateDiffParameters) {
        info->currConfig
            .transformedSequenceConfigurations[info->currStreamIndex]
            .diffCodingEnabled = transID;
        if (transID) {
            info->stack.push(info->stack.top());
            gabac::getTransformation(
                gabac::SequenceTransformationId::diff_coding)
...
void getOptimumOfLutEnabled(const AnalysisConfiguration& aconf,
    ...
    for (const auto& transID : aconf.candidateLutOrder) {
        ...
        int64_t smin, smax;
        if (aconf.maxValue == 0) {
            getMinMax(info->stack.top().streams.front(), &min, &max, &smin,
                      &smax);
        } else {
            max = getMaxOfStream(info->currConfig.sequenceTransformationId,
                                 info->currStreamIndex, aconf.maxValue);
        ...
        info->numConfigs += 1;
        gabac::encode_cabac(gabac::BinarizationId::BI, {bits0},
                            gabac::ContextSelectionId::bypass,
                            &info->stack.top().streams[1]);

        info->currSequenceSize +=
            sizeof(uint32_t) + info->stack.top().streams[1].size();
        ...
        if (info->currConfig
            ...
            info->numConfigs += 1;
            gabac::encode_cabac(gabac::BinarizationId::BI, {bits1},
                                gabac::ContextSelectionId::bypass,
                                &info->stack.top().streams[2]);

            info->currSequenceSize +=
                sizeof(uint32_t) + info->stack.top().streams[2].size();
        }
...
void getOptimumOfTransformedStream(const AnalysisConfiguration& aconf,
    ...
    for (const auto& transID : aconf.candidateLUTCodingParameters) {
        info->currConfig
            .transformedSequenceConfigurations[info->currStreamIndex]
            .lutTransformationEnabled = transID;
...
void getOptimumOfSequenceTransform(const AnalysisConfiguration& aconf,
    ...
    const std::vector<uint32_t>* params[] = {
        &candidateDefaultParameters, &candidateDefaultParameters,
        &aconf.candidateMatchCodingParameters,
        &aconf.candidateRLECodingParameters};
    for (auto const& p :
         *(params[unsigned(info->currConfig.sequenceTransformationId)])) {
        info->stack.push(info->stack.top());

        info->currConfig.sequenceTransformationParameter = p;

        gabac::getTransformation(info->currConfig.sequenceTransformationId)
            .transform({p}, &info->stack.top().streams);
        ...
        for (unsigned i = 0; i < info->stack.top().streams.size(); ++i) {
            ...
            info->currConfig.transformedSequenceConfigurations[i] =
                info->bestSeqConfig;
            if (info->bestSequenceSize == std::numeric_limits<size_t>::max()) {
                info->ioconf->log(gabac::IOConfiguration::LogLevel::DEBUG)
                    ...
                    << info->currStreamIndex << " of transformation"
                    << unsigned(info->currConfig.sequenceTransformationId)
                    << " in word size " << info->currConfig.wordSize
                    << " Skipping!" << std::endl;
            ...
            if (info->currTotalSize >= info->bestTotalSize) {
                info->ioconf->log(gabac::IOConfiguration::LogLevel::TRACE)
                    << "Skipping. Bitstream already larger than permitted."
                    << std::endl;
                info->stack.pop();
                break;
            }
        ...
        if (info->currTotalSize < info->bestTotalSize) {
            info->ioconf->log(gabac::IOConfiguration::LogLevel::DEBUG)
                << "Found configuration compressing to " << info->currTotalSize
                << " bytes." << std::endl;
            info->bestTotalSize = info->currTotalSize;
...
void getOptimumOfSymbolSequence(const AnalysisConfiguration& aconf,
    ...
    for (const auto& transID : aconf.candidateSequenceTransformationIds) {
        info->ioconf->log(gabac::IOConfiguration::LogLevel::INFO)
            << "Transformation " << unsigned(transID) << "..." << std::endl;
        info->currConfig.sequenceTransformationId = transID;
        info->currConfig.transformedSequenceConfigurations.resize(
            gabac::getTransformation(transID).wordsizes.size());
        // Core of analysis
        getOptimumOfSequenceTransform(aconf, info);
    }
}
...
void analyze(const IOConfiguration& ioconf,
    ...
    if (!ioconf.blocksize) {
        gabac::StreamHandler::readFull(*ioconf.inputStream,
                                       &info.stack.top().streams.front());
    } else {
        gabac::StreamHandler::readBytes(*ioconf.inputStream, ioconf.blocksize,
                                        &info.stack.top().streams.front());
    }
    ...
    for (const auto& w : wordsize_cand) {
        ...
        if (info.stack.top().streams.front().getRawSize() % w != 0) {
            ioconf.log(gabac::IOConfiguration::LogLevel::WARNING)
                << "Input stream size "
                << info.stack.top().streams.front().getRawSize()
                << " is not a multiple of word size " << w
                << "! Skipping word size." << std::endl;
            continue;
        }

        if (w == 8 && info.stack.top().streams.front().getMaximum() >
                          std::numeric_limits<uint32_t>::max()) {
            ioconf.log(gabac::IOConfiguration::LogLevel::WARNING)
                << "Too large values for wordsize 8" << std::endl;
            continue;
        }
    ...
    ioconf.log(gabac::IOConfiguration::LogLevel::INFO)
        << "filesize:" << info.bestTotalSize
        << ",num_configs:" << info.numConfigs;
}

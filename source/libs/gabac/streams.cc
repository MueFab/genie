...
DataBlockBuffer::DataBlockBuffer(DataBlock *d, size_t pos_i)
    : block(0, 1), pos(pos_i) {
    block.swap(d);
}
...
std::streamsize DataBlockBuffer::xsputn(const char *s, std::streamsize n) {
    ...
    memcpy(
        static_cast<uint8_t *>(block.getData()) + oldSize * block.getWordSize(),
...
std::streamsize DataBlockBuffer::xsgetn(char *s, std::streamsize n) {
    ...
    size_t bytesRead =
        std::min(block.getRawSize() - pos * block.getWordSize(), size_t(n));
    memcpy(s,
           static_cast<uint8_t *>(block.getData()) + pos * block.getWordSize(),
           bytesRead);
...
IBufferStream::IBufferStream(DataBlock *d, size_t pos_i)
    : DataBlockBuffer(d, pos_i), std::istream(this) {}

OBufferStream::OBufferStream(DataBlock *d)

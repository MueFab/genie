...
static void decodeInverseLUT(unsigned bits0, unsigned order,
    ...
    gabac::decode_cabac(gabac::BinarizationId::BI, {bits0},
    ...
    if (order > 0) {
        ...
        gabac::decode_cabac(gabac::BinarizationId::BI, {bits1},
                            gabac::ContextSelectionId::bypass,
...
static void doLUTCoding(bool enabled, unsigned order,
...
static void doEntropyCoding(
    const gabac::TransformedSequenceConfiguration
    ...
    gabac::decode_cabac(transformedSequenceConfiguration.binarizationId,
                        ...
                        wordsize, diffAndLutTransformedSequence);
}

void decode(const IOConfiguration &ioConf,
    ...
    while (ioConf.inputStream->peek() != EOF) {
        ...
        size_t numTransformedSequences =
            gabac::getTransformation(enConf.sequenceTransformationId)
                .wordsizes.size();
        ...
        for (size_t i = 0; i < numTransformedSequences; i++) {
            ...
            auto transformedSequenceConfiguration =
                enConf.transformedSequenceConfigurations.at(i);
            ...
            if (transformedSequenceConfiguration.lutTransformationEnabled) {
                decodeInverseLUT(
                    ...
                    ioConf.inputStream, &lutTransformedSequences[1],
                    &lutTransformedSequences[2]);
            }

            uint8_t wsize =
                getTransformation(enConf.sequenceTransformationId).wordsizes[i];
            if (wsize == 0) {
                wsize = static_cast<uint8_t>(enConf.wordSize);
            }

            doEntropyCoding(enConf.transformedSequenceConfigurations[i], wsize,
                            ioConf.inputStream, &lutTransformedSequences[0]);

            doDiffCoding(
                enConf.transformedSequenceConfigurations[i].diffCodingEnabled,
                &lutTransformedSequences[0]);

            doLUTCoding(enConf.transformedSequenceConfigurations[i]
        ...
        getTransformation(enConf.sequenceTransformationId)
            .inverseTransform({enConf.sequenceTransformationParameter},
                              &transformedSequences);
        // GABACIFY_LOG_TRACE << "Decoded sequence of length: " <<
        // transformedSequences[0].size();

        gabac::StreamHandler::writeBytes(*ioConf.outputStream,

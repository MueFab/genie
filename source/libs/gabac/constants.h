 ...
 */
using SequenceTransform = std::function<void(
    const std::vector<uint64_t>& param, std::vector<gabac::DataBlock>* const)>;
...
struct TransformationProperties {
    ...
    SequenceTransform
        transform; /**< @brief Function pointer to transformation */
...
struct BinarizationProperties {
    ...
    SignedBinarizationBorder
        min; /**< @brief Minimum supported symbol with a particular parameter */
    SignedBinarizationBorder
        max; /**< @brief Maximum supported symbol with a particular parameter */

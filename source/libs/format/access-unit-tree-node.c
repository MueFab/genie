...
void freeAccessUnitTreeNode(ldns_rbnode_t *node);
...
void freeAccessUnitTreeNode(ldns_rbnode_t *node) { free(node); }

int accessUnitTreeNodeCompareFunction_Seq_Au_Class(const void *thisNode, const void *thatNode) {
    return accessUnitTreeNodeCompareFunctionCasted_Seq_Au_Class((const DataUnitAccessUnitTreeNodeKey *)thisNode,

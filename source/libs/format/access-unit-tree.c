...
AccessUnitTree* initAccessUnitTree() {
    ...
    accessUnitTree->data = (DataUnitAccessUnitTreeNodeData*)malloc(sizeof(DataUnitAccessUnitTreeNodeData) *
                                                                   accessUnitTree->allocated_elements);
    if (accessUnitTree->data == NULL) {
        goto error;
    }
    accessUnitTree->keys = (DataUnitAccessUnitTreeNodeKey*)malloc(sizeof(DataUnitAccessUnitTreeNodeKey) *
                                                                  accessUnitTree->allocated_elements);
...
bool addElement(AccessUnitTree* accessUnitTree, DataUnitAccessUnitTreeNodeKey dataUnitAccessUnitTreeNodeKey,
    ...
    if (accessUnitTree->allocated_elements == accessUnitTree->currentNumberElements) {
        DataUnitAccessUnitTreeNodeData* newDatas =
            realloc(accessUnitTree->data, accessUnitTree->allocated_elements * 2);
        DataUnitAccessUnitTreeNodeKey* newKeys = realloc(accessUnitTree->data, accessUnitTree->allocated_elements * 2);
        if (newDatas != NULL) {
    ...
    ldns_rbtree_insert(accessUnitTree->AccessUnitTree, new_node);
    return true;
}
...
ldns_rbnode_t* getNextAccessUnitNode(ldns_rbnode_t* currentNode) { return ldns_rbtree_next(currentNode); }

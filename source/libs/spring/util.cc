...
bool check_id_pattern(const std::string &id_1, const std::string &id_2, const uint8_t paired_id_code) {
...
void write_dna_in_bits(const std::string &read, std::ofstream &fout) {
    ...
    for (int i = 0; i < readlen / 4; i++) {
        byte = 0;
        for (int j = 0; j < 4; j++) byte = byte * 4 + dna2int[(uint8_t)read[4 * i + j]];
        fout.write((char *)&byte, sizeof(uint8_t));
    }
    if (readlen % 4 != 0) {
        int i = readlen / 4;
        byte = 0;
        for (int j = 0; j < readlen % 4; j++) byte = byte * 4 + dna2int[(uint8_t)read[4 * i + j]];
        fout.write((char *)&byte, sizeof(uint8_t));
    }
    return;
}

...
namespace spring {
    ...
    generate_read_streams(const std::string &temp_dir, const compression_params &cp, dsg::StreamSaver &st) {
        if (!cp.paired_end)
    ...
    std::vector<std::map<uint8_t, std::map<uint8_t, std::string>>>
    generate_read_streams_se(const std::string &temp_dir, const compression_params &cp, dsg::StreamSaver &st) {
        ...
        uint64_t blocks = uint64_t(std::ceil(float(num_reads) / num_reads_per_block));
...
#pragma omp parallel for ordered
        ...
        for (uint64_t block_num = 0; block_num < blocks; block_num++) {
            ...
            for (uint64_t i = start_read_num; i < end_read_num; i++) {
                if (flag_arr[i] == true) {
                        ...
                        for (uint16_t j = 0; j < noise_len_arr[i]; j++) {
                            ...
                            if (j == 0)
                                ...
                                subseq_vector[3][1].push_back(
                                        noisepos_arr[pos_in_noise_arr[i] + j] - 1); // decoder adds +1
                            subseq_vector[4][0].push_back(0); // mmtype = Substitution
                            subseq_vector[4][1].push_back(
                                    char_to_int[(uint8_t) noise_arr[pos_in_noise_arr[i] + j]]);
                        }
                    ...
                    for (uint64_t j = 0; j < read_length_arr[i]; j++) {
                        subseq_vector[6][0].push_back(
                                char_to_int[(uint8_t) unaligned_arr[pos_arr[i] + j]]); // ureads
                    }
            ...
            for (auto arr : subseq_indices) {
                ...
                streamsAU.streams[arr[0]][arr[1]] = gabac::DataBlock(&subseq_vector[arr[0]][arr[1]]);
                st.compress(filename, &streamsAU.streams[arr[0]][arr[1]]);
            }
#ifdef GENIE_USE_OPENMP
#pragma omp ordered
#endif
            {
                for (auto arr : subseq_indices) {
                    std::string filename = file_subseq_prefix + "." + std::to_string(block_num) + "." +
                                           std::to_string(arr[0]) + "." + std::to_string(arr[1]);
                    listDescriptorFiles[arr[0]][arr[1]] = filename;
                    if (streamsAU.streams[arr[0]][arr[1]].getRawSize()) {
                        st.pack(streamsAU.streams[arr[0]][arr[1]], filename);
                    }
                }
...
// in same genomic record.
        ...
        for (uint32_t i = 0; i < num_reads; i++) {
            ...
            if (!already_seen[current]) {
                ...
                if (already_seen[pair]) {
                    if (block_num[pair] == current_block_num && (pos_arr[current] >= pos_arr[pair]) &&
                        (pos_arr[current] - pos_arr[pair] < 32768)) {
                        // put in same record
                    ...
                    if (!flag_arr[pair]) {
                        ...
                        if (!flag_arr[current]) {
                            ...
                            read_index_genomic_record.push_back(
        ...
        for (uint32_t i = 0; i < block_start.size(); i++) {
            ...
            for (uint32_t j = block_start[i]; j < block_end[i]; j++) {
                ...
                uint32_t pair = (current < num_reads_by_2) ? (current + num_reads_by_2) : (current - num_reads_by_2);
                if ((block_num[current] == block_num[pair]) &&
                    (genomic_record_index[pair] == genomic_record_index[current])) {
...
#pragma omp parallel for ordered schedule(dynamic)
        ...
        for (uint64_t cur_block_num = 0; cur_block_num < block_start.size(); cur_block_num++) {
            ...
            for (uint32_t i = block_start[cur_block_num]; i < block_end[cur_block_num]; i++) {
                ...
                uint32_t pair = (current < num_reads_by_2) ? (current + num_reads_by_2) : (current -
                                                                                           num_reads_by_2);
                ...
                if ((block_num[current] == block_num[pair]) &&
                    ...
                    if (flag_arr[current] == false) {
                        ...
                        for (uint64_t j = 0; j < read_length_arr[current]; j++) {
                            subseq_vector[6][0].push_back(
                                    char_to_int[(uint8_t) unaligned_arr[pos_arr[current] + j]]); // ureads
                        }
                        for (uint64_t j = 0; j < read_length_arr[pair]; j++) {
                            subseq_vector[6][0].push_back(
                                    char_to_int[(uint8_t) unaligned_arr[pos_arr[pair] + j]]); // ureads
                        }
                        subseq_vector[0][0].push_back(seq_end - prevpos); // pos
                        subseq_vector[1][0].push_back(0); // rcomp
                        subseq_vector[1][0].push_back(0); // rcomp
                        ...
                        subseq_vector[8][0].push_back(0); // pair decoding case same_rec
                        subseq_vector[8][1].push_back(!(read_1_first) + 2 * delta); // pair
                        count_same_rec[omp_get_thread_num()]++;
                    }
                } else {
                    ...
                    if (flag_arr[current] == true) {
                        ...
                        if (noise_len_arr[current] == 0)
                            ...
                            for (uint16_t j = 0; j < noise_len_arr[current]; j++) {
                                ...
                                if (j == 0)
                                    subseq_vector[3][1].push_back(
                                            noisepos_arr[pos_in_noise_arr[current] + j]); // mmpos
                                else
                                    subseq_vector[3][1].push_back(
                                            noisepos_arr[pos_in_noise_arr[current] + j] - 1); // mmpos
                                subseq_vector[4][0].push_back(0); // mmtype = Substitution
                                subseq_vector[4][1].push_back(
                                        char_to_int[(uint8_t) noise_arr[pos_in_noise_arr[current] + j]]);
                            }
                            subseq_vector[3][0].push_back(1);
                        }
                    } else {
                    ...
                    if (same_block)
                        count_split_same_AU[omp_get_thread_num()]++;
                    else
                        count_split_diff_AU[omp_get_thread_num()]++;
...
#pragma omp ordered
        ...
        std::cout << "count_split_same_AU: "
                  << std::accumulate(count_split_same_AU.begin(), count_split_same_AU.end(), 0) << "\n";
        std::cout << "count_split_diff_AU: "
                  << std::accumulate(count_split_diff_AU.begin(), count_split_diff_AU.end(), 0) << "\n";

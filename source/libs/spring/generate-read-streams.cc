                ...
                if (data.noise_len_arr[i] == 0)
                    ...
                    for (uint16_t j = 0; j < data.noise_len_arr[i]; j++) {
                        ...
                        if (j == 0)
                            ...
                            subseqData->subseq_vector[3][1].push_back(
...
#pragma omp parallel for num_threads(num_thr)
    ...
    std::vector<std::map<uint8_t, std::map<uint8_t, std::string>>>
    generate_and_compress_se(const se_data &data, dsg::StreamSaver &st, bool analyze) {
...
#pragma omp ordered
    ...
    std::vector<std::map<uint8_t, std::map<uint8_t, std::string>>>
    generate_read_streams_se(const std::string &temp_dir, const compression_params &cp, bool analyze, dsg::StreamSaver &st) {
        se_data data;
    ...
    void generateBlocksPE (const se_data& data, pe_block_data* bdata) {
        ...
        for (uint32_t i = 0; i < data.cp.num_reads; i++) {
            ...
            if (!already_seen[current]) {
                ...
                uint32_t pair = (current < data.cp.num_reads/2) ? (current + data.cp.num_reads/2) : (current - data.cp.num_reads/2);
    ...
    void generate_streams_pe(const se_data& data, const pe_block_data& bdata, uint64_t cur_block_num, subseq_data* out, pe_statistics* stats){
        ...
        for (auto arr : subseq_indices)
            out->subseq_vector[arr[0]][arr[1]] = std::vector<int64_t>();
        //clear vectors
        for (auto arr : subseq_indices)
            out->subseq_vector[arr[0]][arr[1]].clear();
        ...
        for (uint32_t i = bdata.block_start[cur_block_num]; i < bdata.block_end[cur_block_num]; i++) {
            ...
            uint32_t pair = (current < data.cp.num_reads/2) ? (current + data.cp.num_reads/2) : (current -
                    data.cp.num_reads/2);

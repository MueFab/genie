...
#pragma omp parallel num_threads(cp.num_thr)
#endif
            {
                ...
                uint32_t num_reads_thr =
                    std::min((uint64_t)num_reads_read, (tid + 1) * num_reads_per_block) - tid * num_reads_per_block;
                ...
                if (!done_loop) {
                    ...
                    for (uint32_t i = tid * num_reads_per_block; i < tid * num_reads_per_block + num_reads_thr; i++) {
                        ...
                        if (len > MAX_READ_LEN) {
                            std::cerr << "Max read length without long mode is " << MAX_READ_LEN
                        ...
                        // mark reads with N
                        //            if (!cp.long_flag)
                        read_contains_N_array[i] = (fastqRecords[i].sequence.find('N') != std::string::npos);

                        // Write read length to a file (for long mode)
                        //            if (cp.long_flag)
                        //              fout_readlength.write((char *)&read_lengths_array[i],
                        //                                    sizeof(uint32_t));
                        if (j == 0) id_array_1[i] = fastqRecords[i].title;  // store for later comparison with id_2
                        if (j == 1 && paired_id_match_array[tid])
                            paired_id_match_array[tid] =
                                check_id_pattern(id_array_1[i], fastqRecords[i].title, paired_id_code);
                    }
                    ...
                    //            quantize_quality_qvz(quality_array + tid * num_reads_per_block, num_reads_thr,
            ...
            if (cp.paired_end && (j == 1)) {
                if (paired_id_match)
                    for (int tid = 0; tid < num_threads; tid++) paired_id_match &= paired_id_match_array[tid];
            ...
            for (uint32_t i = 0; i < num_reads_read; i++) fout_quality[j] << fastqRecords[i].qualityScores << "\n";
            // write ids to file
            for (uint32_t i = 0; i < num_reads_read; i++) fout_id[j] << fastqRecords[i].title << "\n";
            //        }
            //      }
            num_reads[j] += num_reads_read;
            max_readlen =
                std::max(max_readlen, *(std::max_element(read_lengths_array, read_lengths_array + num_reads_read)));
        }
        if (cp.paired_end)
    ...
    if (cp.preserve_id && cp.paired_end) std::cout << "Paired id match code: " << (int)cp.paired_id_code << "\n";
}

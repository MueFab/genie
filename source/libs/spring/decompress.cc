...
std::pair<std::vector<utils::FastqRecord>, std::vector<bool>> decode_streams(
    decoded_desc_t &dec, const std::vector<std::array<uint8_t, 2>> &subseq_indices, bool paired_end) {
    ...
    for (; subseq_it[12][0] != dec.subseq_vector[12][0].end(); ++subseq_it[12][0]) {
        ...
        if (rtype == 5) {
            ...
            uint32_t rlen[2];
            for (int i = 0; i < number_of_record_segments; i++) rlen[i] = (uint32_t)(*(subseq_it[7][0]++));  // rlen
...
#pragma omp ordered
#endif
        {
            ...
            for (size_t i = 0; i < decoded_records.first.size(); i++) {
                ...
                if (cp.paired_end)
                    if (!decoded_records.second[i]) first_file_flag = false;

...
# Gather all C++ files
cpp_files=()
cpp_files+=("$git_root_dir"/source/genie/*.cpp)
cpp_files+=("$git_root_dir"/source/genie/*.h)
...
# cppcheck arguments
cppcheck_args=""
cppcheck_args="$cppcheck_args --enable=all"
cppcheck_args="$cppcheck_args --inconclusive"
cppcheck_args="$cppcheck_args --language=c++"
cppcheck_args="$cppcheck_args --std=posix"
cppcheck_args="$cppcheck_args --suppress=missingIncludeSystem"
cppcheck_args="$cppcheck_args --suppress=unusedFunction"
cppcheck_args="$cppcheck_args --suppress=unmatchedSuppression"
printf "cppcheck arguments: %s\\n" "$cppcheck_args"

# Do the work
for cpp_file in "${cpp_files[@]}"; do
    printf "Running cppcheck on: %s\\n" "$cpp_file"
    "$cppcheck" $cppcheck_args -I "$git_root_dir"/source "$cpp_file"
done

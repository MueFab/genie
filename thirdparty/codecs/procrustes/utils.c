...
MB* mb_new(const size_t size,void* const embedding) { /*DEC*/
  ...
  if (embedding) {
    ...
    res->allocated=false;
    if (mb_is_verbose) {
      printf("(mb_new): Created embedded buffer [%p-%p] (%ld)\n",res->buf,res->buf+res->len,res->len);
      fflush(stdout);
    }
  } else {
    if (size) {
      ...
      if (mb_is_verbose) {
        printf("(mb_new): Allocated [%p-%p] (%ld)\n",res->buf,res->buf+res->len,res->len);
        fflush(stdout);
      }      
    }
    res->allocated=true;
...
MB* mb_read(const char* const fname) { /*DEC*/
  ...
  if ((res->buf=malloc(res->len))) {
    if (mb_is_verbose) {
      printf("(mb_read): Allocated [%p-%p] (%ld)\n",res->buf,res->buf+res->len,res->len);
      fflush(stdout);
    }
    // Workaround to bypass the mysterious 2^31 limit
...
void mb_delete(MB* const m) { /*DEC*/
  if (m->allocated) {
    free(m->buf);
    if (mb_is_verbose) {
      printf("(mb_delete): Deallocated [%p-%p] (%ld)\n",m->buf,m->buf+m->len,m->len);
      fflush(stdout);
    }
  }
  free(m);
...
void mb_verbose(const bool is_verbose) {
  mb_is_verbose=is_verbose;
  return;
}

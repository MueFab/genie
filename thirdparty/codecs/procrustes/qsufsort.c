...
static ssa_t* I,                /* group array, ultimately suffix array. */
...
static void update_group(ssa_t *pl,ssa_t *pm) {
  ssa_t g;
  g=pm-I;                       /* group number. */
...
static void select_sort_split(ssa_t *p,ssa_t n) {
  ssa_t *pa, *pb, *pi, *pn;
  ssa_t f, v, tmp;
  //
...
static ssa_t choose_pivot(ssa_t *p,ssa_t n) {
  ssa_t *pl, *pm, *pn;
  ssa_t s;
  //
...
static void sort_split(ssa_t *p,ssa_t n) {
  ssa_t *pa, *pb, *pc, *pd, *pl, *pm, *pn;
  ssa_t f, v, s, t, tmp;
  //
...
static void bucketsort(ssa_t *x,ssa_t *p,ssa_t n,ssa_t k) {
  ssa_t *pi, i, c, d, g;
  //
  for (pi=p; pi<p+k; ++pi)
...
/* Transforms the alphabet of x by attempting to aggregate several symbols into
   ...
   SSA_T_MAX, the maximum number of symbols are aggregated into one.
   
   Output: Returns an integer j in the range 1...q representing the size of the
   new alphabet. If j<=n+1, the alphabet is compacted. The global variable r is
   set to the number of old symbols grouped into one. Only x[n] is 0. */

static ssa_t transform(ssa_t *x,ssa_t *p,ssa_t n,ssa_t l,ssa_t k,ssa_t q) {
  ssa_t b, c, d, e, i, j, m, s;
  ssa_t *pi, *pj;
  //
  for (s=0, i=k-l; i; i>>=1)
    ++s;                        /* s is number of bits in old symbol. */
  e=SSA_T_MAX>>s;               /* e is for overflow checking. */
...
void suffixsort(ssa_t* x,ssa_t* p,ssa_t n,ssa_t l,ssa_t k) {
  ...
  if (n>=k-l) {                 /* if bucketing possible, */
    j=transform(V,I,n,l,k,n);
    bucketsort(V,I,n,j);        /* bucketsort on first r positions. */
  } else {
    transform(V,I,n,l,k,/*SSA_T_MAX*/k-l);
    for (i=0;i<=n;++i)

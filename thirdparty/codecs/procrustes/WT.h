...
// If embedding_excess!=0 we allocate the needed storage plus the excess, and return all in embedding_buffer.
//  All other buffers for this object will be allocated as embeded ones 
WT* wt_new(sa_t n,ch_t (*init)(const sa_t),sa_t red_isa_0,
           const sa_t superblock_size,const uint8_t block_size,bool verbose,
           size_t embedding_excess,MB* const embedding_buffer);
...
sa_t wt_rank(WT* const wt,ch_t c,sa_t i); // Not needed in actual practice

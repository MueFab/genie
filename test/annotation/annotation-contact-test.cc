...
struct ContactTestDetails {
  ContactTestDetails(std::string casename, std::string contactin, bool REMOVE_UNALIGNED_REGION,
                     bool TRANSFORM_MASK, bool ENA_DIAG_TRANSFORM,
                     bool ENA_BINARIZATION, bool NORM_AS_WEIGHT,
                     bool MULTIPLICATIVE_NORM, uint32_t TILE_SIZE, uint32_t MULT = 1u )
...
TEST_P(AnnotationContactTests, contactparameterset) {
  ...
  std::string outputFilename = testParams.outputfile + "-output";
 ...
 // cmParameters.NORM_AS_WEIGHT = false;
 // cmParameters.MULTIPLICATIVE_NORM = false;
...
// contactin 
// REMOVE_UNALIGNED_REGION, TRANSFORM_MASK,  ENA_DIAG_TRANSFORM,  ENA_BINARIZATION, NORM_AS_WEIGHT,  MULTIPLICATIVE_NORM,TILE_SIZE
INSTANTIATE_TEST_SUITE_P(
    testoutputs, AnnotationContactTests,
    ::testing::Values(
        ContactTestDetails("CASE01",
        ...
        ContactTestDetails(
            "CASE09",
            "GSE63525_GM12878_insitu_primary_30.hic-raw-250000-21_21.cont",
            true, false, true, true, false, false, 1000, 5u)
                          ));

...
TEST(DiffCodingTest, transformDiffCoding) {
...

    {
        // Corner cases
        genie::util::DataBlock transformedSymbols(0, 8);
        transformedSymbols = {0,
static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
                              static_cast<uint64_t>(std::numeric_limits<int64_t>::max())
+ 1, 0}; genie::util::DataBlock expectedTransformedSymbols(0, 8);
    ...
    }
    {
        ...
        gabac_tests::fillVectorRandomUniform(0,
std::numeric_limits<int64_t>::max(), &symbols);
        EXPECT_NO_THROW(genie::entropy::gabac::transformDiffCoding(&transformedSymbols));
    }

    {
        ...
        genie::util::DataBlock transformedSymbols(0, 8);
        transformedSymbols = {uint64_t(std::numeric_limits<int64_t>::max()) + 2,
0};
        EXPECT_NO_THROW(genie::entropy::gabac::transformDiffCoding(&transformedSymbols));
        EXPECT_NO_THROW(genie::entropy::gabac::inverseTransformDiffCoding(&transformedSymbols));
        EXPECT_EQ(transformedSymbols.get(0),
uint64_t(std::numeric_limits<int64_t>::max()) + 2);
...

    {
        ...
        genie::util::DataBlock transformedSymbols(0, 8);
        transformedSymbols = {0, uint64_t(std::numeric_limits<int64_t>::max()) +
1};
        EXPECT_NO_THROW(genie::entropy::gabac::transformDiffCoding(&transformedSymbols));
        EXPECT_NO_THROW(genie::entropy::gabac::inverseTransformDiffCoding(&transformedSymbols));
        EXPECT_EQ(transformedSymbols.get(0), 0);
        EXPECT_EQ(transformedSymbols.get(1),
...
TEST(DiffCodingTest, inverseTransformDiffCoding) {
    ...
    }
    {
        ...
        genie::util::DataBlock transformedSymbols(0, 8);
        transformedSymbols = {100, uint64_t(-10), uint64_t(-10), uint64_t(-10),
...
TEST(DiffCodingTest, roundTripCoding) {
    ...
    symbols.resize(largeTestSize);
    gabac_tests::fillVectorRandomUniform(0, std::numeric_limits<int64_t>::max(),
&symbols); transformedSymbols = symbols;
    EXPECT_NO_THROW(genie::entropy::gabac::transformDiffCoding(&transformedSymbols));
    EXPECT_NO_THROW(genie::entropy::gabac::inverseTransformDiffCoding(&transformedSymbols));
